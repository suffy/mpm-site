...
class Model_master_data extends CI_Model 
{
    ...
    private $batch_size = 1000; // Process in batches
    ...
    public function get_penta_sales_optimized($token, $tahun, $bulan)
    {
        try {
            // API call with timeout and retry logic
            $api_data = $this->_call_penta_api('sales', $token, $tahun, $bulan);
            ...
            // Create log entry
            $signature = 'penta-sales-' . uniqid() . '-' . date('Ymd-His');
            $log_data = array(
                ...
                "bulan" => $bulan,
                "signature" => $signature,
                "total_records" => count($api_data['data'])
            );
            ...
            $this->db->insert('site.penta_log_sales', $log_data);
            ...
            $this->_process_sales_data_in_batches($api_data['data'], $id_log, $signature);
            
            return $id_log;
            
        } catch (Exception $e) {
            log_message('error', 'Penta Sales API Error: ' . $e->getMessage());
            return false;
        }
    }
    
    public function get_penta_sales_ext_optimized($token, $tahun, $bulan)
    {
        try {
            $api_data = $this->_call_penta_api('sales_ext', $token, $tahun, $bulan);
            
            if (!$api_data || !isset($api_data['data'])) {
                return false;
            }
            
            $signature = 'penta-sales-ext-' . uniqid() . '-' . date('Ymd-His');
            $log_data = array(
                "created_at" => $this->created_at,
                "created_by" => $this->userid,
                "token" => $token,
            ...
            if (!$id_log) {
                throw new Exception('Failed to create ext log entry');
            }
            
            // Process ext data in batches
            $this->_process_sales_ext_data_in_batches($api_data['data'], $id_log, $signature);
            
            return $id_log;
            
        } catch (Exception $e) {
            log_message('error', 'Penta Sales Ext API Error: ' . $e->getMessage());
            return false;
        }
    }
    
    // public function join_sales_optimized($id_log, $id_log_ext)
    // {
    //     try {
    //         // PHP 5 compatible query with proper escaping


    //         // karena jumlah penta_sales_origin 28000 dan penta_sales_origin_ext 5000, maka proses insert di pecah menjadi 
    ...
    //             SELECT 	
    //                 NULL as id,
    //                 a.bulan, a.tahun, a.principal_id, a.area_id, a.nama_area, 
    //                 a.tanggal_invoice, a.nomor_invoice, a.nomor_sales_order, 
    //                 a.customer_po_number, a.kode_outlet, a.kode_outlet_lama,
    //                 a.nama_outlet, a.category_produk, a.sales_order_line, 
    //                 a.kode_produk, a.kode_produk_lama, a.inventory_item_id, 
    //                 a.item_id_vend, a.id_item_sapora, a.category_product_principal,
    //                 a.nama_produk, a.qty, a.uom, a.price, a.total_disc, 
    //                 a.total_vat, a.total_gross, a.total_net, a.bonus, 
    //                 a.discount_value_distributor, a.discount_value_prinsipal,
    //                 a.discount_value_extra, a.discount_persen_distributor, 
    //                 a.discount_persen_prinsipal, a.discount_persen_extra, 
    //                 a.nomor_discount_distributor, a.nomor_discount_prinsipal, 
    //                 a.nomor_discount_extra, a.type_data, a.nama_sales, 
    //                 a.batch, a.type_promo, a.keterangan_promo,
    //                 b.address, b.province, b.city, b.npwp, b.channel, 
    //                 a.signature
    //             FROM site.penta_sales_origin a 
    //             LEFT JOIN (
    //                 SELECT nomor_invoice, address, province, city, npwp, channel
    //                 FROM site.penta_sales_ext_origin 
    //                 WHERE id_log = " . (int)$id_log_ext . "
    //             ) b ON a.nomor_invoice = b.nomor_invoice
    //             WHERE a.id_log = " . (int)$id_log . "
    //             limit 
    //         ";
    ...
    //     } catch (Exception $e) {
    //         log_message('error', 'Join Sales Error: ' . $e->getMessage());
    //         return false;
    //     }
    // }
    ...
    public function join_sales_optimized($id_log, $id_log_ext)
    {
        try {
            // Set batch size
            $batch_size = 1000; // Anda bisa menyesuaikan ukuran batch sesuai kebutuhan
            $offset = 0;
            $total_inserted = 0;
            
            // Dapatkan total records yang akan diproses
            $count_query = "
            ...
            while ($offset < $total_records) {
                // Mulai transaksi untuk setiap batch
                $this->db->trans_begin();
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    log_message('error', 'Batch insert failed at offset: ' . $offset);
                    return false;
                } else {
                    ...
                    // Beri jeda kecil antara batch untuk mengurangi lock contention
                    usleep(100000); // 100ms
                }
            }
    ...
    //     try {
    //         // Set transaction isolation level untuk mengurangi lock contention
    ...
    //         $check_query = "SELECT COUNT(*) as count FROM site.penta_sales_join 
    //                         WHERE id_log_origin = ? OR id_log_ext_origin = ?";
    //         $check_result = $this->db->query($check_query, array($id_log, $id_log_ext));
            
    //         if ($check_result->row()->count > 0) {
    //             log_message('info', 'Data already exists for id_log: ' . $id_log . ', id_log_ext: ' . $id_log_ext);
    //             $this->db->trans_rollback();
    //             return array('status' => 'exists', 'message' => 'Data already processed');
    //         }
            
    //         // Optimized query dengan batching dan proper indexing hints
    //         $insert_query = "
    //             INSERT INTO site.penta_sales_join (
    //                 bulan, tahun, principal_id, area_id, nama_area,
    //                 tanggal_invoice, nomor_invoice, nomor_sales_order,
    //                 customer_po_number, kode_outlet, kode_outlet_lama,
    //                 nama_outlet, category_produk, sales_order_line,
    //                 kode_produk, kode_produk_lama, inventory_item_id,
    //                 item_id_vend, id_item_sapora, category_product_principal,
    //                 nama_produk, qty, uom, price, total_disc,
    //                 total_vat, total_gross, total_net, bonus,
    //                 discount_value_distributor, discount_value_prinsipal,
    //                 discount_value_extra, discount_persen_distributor,
    //                 discount_persen_prinsipal, discount_persen_extra,
    //                 nomor_discount_distributor, nomor_discount_prinsipal,
    //                 nomor_discount_extra, type_data, nama_sales,
    //                 batch, type_promo, keterangan_promo,
    //                 address, province, city, npwp, channel,
    //                 signature, id_log_origin, id_log_ext_origin,
    //                 created_at
    //             )
    //             SELECT 
    //                 a.bulan, a.tahun, a.principal_id, a.area_id, a.nama_area,
    ...
    //                 COALESCE(b.channel, '') as channel,
    //                 a.signature,
    //                 ? as id_log_origin,
    //                 ? as id_log_ext_origin,
    //                 NOW() as created_at
    //             FROM site.penta_sales_origin a USE INDEX (idx_id_log)
    //             LEFT JOIN (
    //                 SELECT DISTINCT 
    //                     nomor_invoice, 
    //                     address, 
    ...
    //                     npwp, 
    //                     channel
    //                 FROM site.penta_sales_ext_origin USE INDEX (idx_id_log)
    //                 WHERE id_log = ?
    //             ) b ON a.nomor_invoice = b.nomor_invoice
    //             WHERE a.id_log = ?
    //             ORDER BY a.id
    ...
    //         $result = $this->db->query($insert_query, array(
    //             $id_log, 
    //             $id_log_ext, 
    //             $id_log_ext, 
    //             $id_log
    ...
    //         if (!$result) {
    //             throw new Exception('Failed to execute insert query: ' . $this->db->error()['message']);
    //         }
    ...
    //         log_message('info', 'Join Sales Success: ' . $affected_rows . ' rows inserted for id_log: ' . $id_log);
    ...
    //         $error_msg = 'Join Sales Error: ' . $e->getMessage();
    //         log_message('error', $error_msg);
            
    //         // Jika lock timeout, tunggu sebentar dan coba lagi
    //         if (strpos($e->getMessage(), 'Lock wait timeout') !== false) {
    //             sleep(1); // Tunggu 1 detik
    //             log_message('info', 'Retrying after lock timeout for id_log: ' . $id_log);
                
    //             // Retry sekali lagi dengan timeout yang lebih pendek
    //             return $this->retry_join_sales($id_log, $id_log_ext);
    //         }
    ...
    // Function untuk retry dengan timeout lebih pendek
    private function retry_join_sales($id_log, $id_log_ext) 
    {
        try {
            // Set timeout lebih pendek untuk retry
            $this->db->query("SET SESSION innodb_lock_wait_timeout = 10");
            
            // Coba lagi dengan logic yang sama tapi lebih simple
            $this->db->trans_start();
            
            $simple_query = "
                INSERT IGNORE INTO site.penta_sales_join (
                    bulan, tahun, principal_id, area_id, nama_area,
                    tanggal_invoice, nomor_invoice, nomor_sales_order,
                    customer_po_number, kode_outlet, kode_outlet_lama,
                    nama_outlet, category_produk, sales_order_line,
                ...
                SELECT 
                    ...
                    NOW() as created_at
                FROM site.penta_sales_origin a
                LEFT JOIN site.penta_sales_ext_origin b ON (a.nomor_invoice = b.nomor_invoice AND b.id_log = ?)
                WHERE a.id_log = ?
                LIMIT 1000
            ";
            
            $result = $this->db->query($simple_query, array($id_log, $id_log_ext, $id_log_ext, $id_log));
            ...
            return array(
                ...
                'affected_rows' => $this->db->affected_rows(),
                'message' => 'Data processed successfully after retry'
            );
    ...
    private function _call_penta_api($endpoint, $token, $tahun, $bulan)
    {
        $url = getenv('PENTA_API') . "list/" . $endpoint . "/" . $tahun . "/" . $bulan;
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'Penta-API-Client/1.0');
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new Exception("JSON Decode Error: " . $this->_get_json_error());
        }
        
        return $data;
    }
    
    private function _get_json_error()
    {
        // PHP 5 compatible JSON error handling
    ...
    private function _process_sales_data_in_batches($data, $id_log, $signature)
    {
        ...
        $required_fields = array(
            'bulan', 'tahun', 'nomor_invoice', 'kode_produk', 
            'nama_produk', 'qty', 'total_gross', 'total_net'
        );
        
        for ($i = 0; $i < $total_records; $i += $this->batch_size) {
            $batch = array_slice($data, $i, $this->batch_size);
            $insert_data = array();
            ...
            foreach ($batch as $item) {
                ...
                foreach ($required_fields as $field) {
                    if (!isset($item[$field])) {
                        log_message('warning', "Missing field " . $field . " in record");
                        $valid = false;
                        break;
                    }
                }
                ...
                // Prepare insert data with proper sanitization - PHP 5 compatible
                $insert_data[] = array(
                    "id_log" => $id_log,
                    "bulan" => (int)$item["bulan"],
                    "tahun" => (int)$item["tahun"],
                    "principal_id" => isset($item["principal_id"]) ? $item["principal_id"] : null,
                    "area_id" => isset($item["area_id"]) ? $item["area_id"] : null,
                    "nama_area" => isset($item["nama_area"]) ? $this->db->escape_str($item["nama_area"]) : '',
                    "tanggal_invoice" => isset($item["tanggal_invoice"]) ? $item["tanggal_invoice"] : null,
                    "nomor_invoice" => $this->db->escape_str($item["nomor_invoice"]),
                    "nomor_sales_order" => isset($item["nomor_sales_order"]) ? $this->db->escape_str($item["nomor_sales_order"]) : '',
                    "customer_po_number" => isset($item["customer_po_number"]) ? $this->db->escape_str($item["customer_po_number"]) : '',
                    "kode_outlet" => isset($item["kode_outlet"]) ? $this->db->escape_str($item["kode_outlet"]) : '',
                    "kode_outlet_lama" => isset($item["kode_outlet_lama"]) ? $this->db->escape_str($item["kode_outlet_lama"]) : '',
                    "nama_outlet" => isset($item["nama_outlet"]) ? $this->db->escape_str($item["nama_outlet"]) : '',
                    "category_produk" => isset($item["category_produk"]) ? $this->db->escape_str($item["category_produk"]) : '',
                    "sales_order_line" => isset($item["sales_order_line"]) ? $item["sales_order_line"] : null,
                    "kode_produk" => $this->db->escape_str($item["kode_produk"]),
                    "kode_produk_lama" => isset($item["kode_produk_lama"]) ? $this->db->escape_str($item["kode_produk_lama"]) : '',
                    "inventory_item_id" => isset($item["inventory_item_id"]) ? $item["inventory_item_id"] : null,
                    "item_id_vend" => isset($item["item_id_vend"]) ? $item["item_id_vend"] : null,
                    "id_item_sapora" => isset($item["id_item_sapora"]) ? $item["id_item_sapora"] : null,
                    "category_product_principal" => isset($item["category_product_principal"]) ? $this->db->escape_str($item["category_product_principal"]) : '',
                    "nama_produk" => $this->db->escape_str($item["nama_produk"]),
                    "qty" => (float)$item["qty"],
                    "uom" => isset($item["uom"]) ? $this->db->escape_str($item["uom"]) : '',
                    "price" => isset($item["price"]) ? (float)$item["price"] : 0,
                    "total_disc" => isset($item["total_disc"]) ? (float)$item["total_disc"] : 0,
                    "total_vat" => isset($item["total_vat"]) ? (float)$item["total_vat"] : 0,
                    "total_gross" => (float)$item["total_gross"],
                    "total_net" => (float)$item["total_net"],
                    "bonus" => isset($item["bonus"]) ? $item["bonus"] : null,
                    "discount_value_distributor" => isset($item["discount_value_distributor"]) ? (float)$item["discount_value_distributor"] : 0,
                    "discount_value_prinsipal" => isset($item["discount_value_prinsipal"]) ? (float)$item["discount_value_prinsipal"] : 0,
                    "discount_value_extra" => isset($item["discount_value_extra"]) ? (float)$item["discount_value_extra"] : 0,
                    "discount_persen_distributor" => isset($item["discount_persen_distributor"]) ? (float)$item["discount_persen_distributor"] : 0,
                    "discount_persen_prinsipal" => isset($item["discount_persen_prinsipal"]) ? (float)$item["discount_persen_prinsipal"] : 0,
                    "discount_persen_extra" => isset($item["discount_persen_extra"]) ? (float)$item["discount_persen_extra"] : 0,
                    "nomor_discount_distributor" => isset($item["nomor_discount_distributor"]) ? $this->db->escape_str($item["nomor_discount_distributor"]) : '',
                    "nomor_discount_prinsipal" => isset($item["nomor_discount_prinsipal"]) ? $this->db->escape_str($item["nomor_discount_prinsipal"]) : '',
                    "nomor_discount_extra" => isset($item["nomor_discount_extra"]) ? $this->db->escape_str($item["nomor_discount_extra"]) : '',
                    "type_data" => isset($item["type_data"]) ? $this->db->escape_str($item["type_data"]) : '',
                    "nama_sales" => isset($item["nama_sales"]) ? $this->db->escape_str($item["nama_sales"]) : '',
                    "batch" => isset($item["batch"]) ? $this->db->escape_str($item["batch"]) : '',
                    "type_promo" => isset($item["type_promo"]) ? $this->db->escape_str($item["type_promo"]) : '',
                    "keterangan_promo" => isset($item["keterangan_promo"]) ? $this->db->escape_str($item["keterangan_promo"]) : '',
                    "created_at" => $this->created_at,
                    "created_by" => $this->userid,
                    "signature" => $signature
                );
            ...
            if (!empty($insert_data)) {
                $this->db->insert_batch('site.penta_sales_origin', $insert_data);
                $processed += count($insert_data);
                
                // Update progress
                log_message('info', "Processed " . $processed . "/" . $total_records . " sales records");
            }
            ...
            if ($i + $this->batch_size < $total_records) {
                usleep(10000); // 10ms delay
            }
        }
        
        // Store processed count in session for reporting
        $CI =& get_instance();
        $CI->session->set_userdata('records_processed', $processed);
        
        return $processed;
    }
    
    private function _process_sales_ext_data_in_batches($data, $id_log, $signature)
    {
        $total_records = count($data);
        $processed = 0;
        ...
        for ($i = 0; $i < $total_records; $i += $this->batch_size) {
            ...
            foreach ($batch as $item) {
                $insert_data[] = array(
                    "id_log" => $id_log,
                    "nomor_invoice" => isset($item["nomor_invoice"]) ? $this->db->escape_str($item["nomor_invoice"]) : '',
                    "tanggal_invoice" => isset($item["tanggal_invoice"]) ? $item["tanggal_invoice"] : null,
                    "bulan" => isset($item["bulan"]) ? (int)$item["bulan"] : 0,
                    "tahun" => isset($item["tahun"]) ? (int)$item["tahun"] : 0,
                    "area_id" => isset($item["area_id"]) ? $item["area_id"] : null,
                    "nama_area" => isset($item["nama_area"]) ? $this->db->escape_str($item["nama_area"]) : '',
                    "kode_outlet" => isset($item["kode_outlet"]) ? $this->db->escape_str($item["kode_outlet"]) : '',
                    "kode_outlet_lama" => isset($item["kode_outlet_lama"]) ? $this->db->escape_str($item["kode_outlet_lama"]) : '',
                    "nama_outlet" => isset($item["nama_outlet"]) ? $this->db->escape_str($item["nama_outlet"]) : '',
                    "address" => isset($item["address"]) ? $this->db->escape_str($item["address"]) : '',
                    "province" => isset($item["province"]) ? $this->db->escape_str($item["province"]) : '',
                    "city" => isset($item["city"]) ? $this->db->escape_str($item["city"]) : '',
                    "npwp" => isset($item["npwp"]) ? $this->db->escape_str($item["npwp"]) : '',
                    "reference_invoice_return" => isset($item["reference_invoice_return"]) ? $this->db->escape_str($item["reference_invoice_return"]) : '',
                    "reference_date_return" => isset($item["reference_date_return"]) ? $item["reference_date_return"] : null,
                    "channel" => isset($item["channel"]) ? $this->db->escape_str($item["channel"]) : '',
                    "created_at" => $this->created_at,
                    "created_by" => $this->userid,
                    "signature" => $signature
                );
            ...
            if (!empty($insert_data)) {
                $this->db->insert_batch('site.penta_sales_ext_origin', $insert_data);
                $processed += count($insert_data);
            }
    ...
    public function get_customer(){
        ...
        if ($proses_truncate) {
            ...
            select 	a.kode_lang,a.nama_lang,a.kodesalur,c.class, a.kode_type, d.nama_type,
                    ...
                    concat(a.kode_comp,a.nocab) as kode,b.branch_name,b.nama_comp,
                    b.sub, left(b.site_code, 3) as kode_comp, right(b.site_code,2) as nocab, concat(7,right(substr(a.kode_lang,4,6) * 1234,5)) as code_approval,erp_updated,'$tgl'
            FROM
            (
                select 	concat(a.kode_comp,a.kode_lang) as kode_lang, 
                        ...
                        a.last_updated as erp_updated, a.status_blacklist, a.kode_comp, a.nocab
                from    data$tahun_sekarang.tblang a                  
                where concat(a.kode_comp, a.nocab) in ('TGR39')
                GROUP BY concat(a.kode_comp,a.kode_lang,nocab)
            )a INNER JOIN site.master_site b on a.kode = b.site_code
            LEFT JOIN
            (
    ...
    {
        $create_index = "
            create index idx_outstanding_deltomed on db_po_new.t_temp_do_po_outstanding(nopo, kodeprod_deltomed, kodeprod)
        ";
    ...
    public function get_po_outstanding_deltomed()
    {
        ...
        $kunci_traffic = "update db_temp.t_traffic a set a.status = 0, a.created_date = '$created_at' where a.id_menu = 77";
    ...
    public function insert_temp_do_deltomed($created_at, $tahun)
    {
        ...
        select 	a.nodo, a.kodedp, a.company, a.kodeprod_delto, a.kodeprod, a.namaprod, a.qty, a.nopo, 
                a.tgldo, a.batch_number, a.ed, '$created_at',297
        ...
        //         str_to_date(a.tgldo,'%d/%m/%Y') as tgldo, a.batch_number, a.tgl_bulan, '$created_at',297
        // from site.t_do_deltomed a
    ...
    public function insert_temp_do_us($created_at, $tahun)
    {
        $query = "
            ...
            select 	a.id, a.nodo, a.tgldo, a.kodeprod,a.nopo,
                    if(b.satuan_box is null, a.qty_pemenuhan, a.qty_pemenuhan * b.satuan_box) as qty_pemenuhan,
                    a.batch_number,
                    '$created_at',297
            FROM
            (
                select 	a.id, a.nodo, a.tgldo,
                        a.kodeprod, a.banyak as qty_pemenuhan, a.nopo, a.batch_number
    ...
    public function insert_po_outstanding_deltomed($created_at, $tahun)
    {
        ...
        $query = "
            ...
            select 	b.branch_name, b.nama_comp, a.company, a.tglpo,  a.nopo,  a.tipe, a.kodeprod,
                    a.namaprod, a.banyak, 
                    sum(c.qty) as qty_pemenuhan, a.harga, (a.banyak*a.harga) as value_po,
                    (sum(c.qty) * a.harga) as value_pemenuhan, (a.berat * a.banyak_karton) as berat, a.volume,
                    c.tgldo, c.nodo, (sum(c.qty) / a.banyak) * 100 as fulfilment,
                    datediff(c.tgldo, a.tglpo) as lead_time_proses_do,
                    a.po_ref, a.note, a.tanggal_terima, datediff(a.tanggal_terima,c.tgldo) as leadtime_proses_kirim,
                    (a.banyak - sum(c.qty)) as outstanding_po, a.kode_alamat,
                    CONCAT('[', 
                        GROUP_CONCAT(
                            CONCAT('{\"nodo\":\"',c.nodo, 
                                '\",\"tgldo\":\"', c.tgldo, 
                                '\",\"kodeprod\":\"', a.kodeprod, 
                                '\",\"kodeprod_deltomed\":\"', c.kodeprod_deltomed, 
                                '\",\"batch_number\":\"', c.batch_number,
                                '\",\"qty\":\"', c.qty, 
                                '\",\"tanggal_terima\":\"', ifnull(d.tanggal_terima, ''),'\"'
                                '}')
                            SEPARATOR ','
                        ), 
                    ']') AS data_surat_jalan,
            ...
            from db_po_new.`t_temp_report_po_update` a left join (
                    ...
                    select 	a.nodo, a.kodedp, a.company, a.kodeprod, 
                                a.kodeprod_deltomed, a.namaprod, a.qty, a.nopo, tgldo, a.batch_number, a.tgl_bulan 
                    from db_po_new.t_temp_do_po_outstanding a
                )c on a.nopo = c.nopo and a.kodeprod = c.kodeprod left join (
                    select a.nodo, a.batch_number, a.kodeprod, a.qty, a.tanggal_terima
                    from site.konfirmasi_do a
                    where a.deleted_at is null
                )d on c.nodo = d.nodo and c.batch_number = d.batch_number and a.kodeprod = d.kodeprod
    ...
    public function truncate_do_po_new()
    {
        $query = "
            truncate table db_po_new.t_do_us
        ";
        return $this->db->query($query);
    }
    ...
    {
        $query = "
            insert into db_po_new.t_do_us
            select * from db_po.t_do_us
        ";
        return $this->db->query($query);
    }
    ...
    public function insert_po_outstanding_us($created_at, $tahun)
    {
        ...
        $query = "
            ...
            select 	b.branch_name, b.nama_comp, a.company, a.tglpo, a.nopo,  a.tipe, a.kodeprod, 
                    a.namaprod, a.banyak, sum(c.qty) as qty_pemenuhan,  a.harga, (a.banyak*a.harga) as value_po,
                    (sum(c.qty) * a.harga) as value_pemenuhan, (a.berat * a.banyak_karton) as berat, a.volume,
                    c.tgldo, c.nodo, (sum(c.qty) / a.banyak) * 100 as fulfilment,
                    datediff(c.tgldo, DATE_FORMAT(a.tglpo,'%Y-%m-%d')) as lead_time_proses_do,
                    a.po_ref, a.note, a.tanggal_terima, datediff(a.tanggal_terima,c.tgldo) as leadtime_proses_kirim,
                    (a.banyak - sum(c.qty)) as outstanding_po, a.kode_alamat, 
                    CONCAT('[', 
                        GROUP_CONCAT(
            ...
            from 	db_po_new.`t_temp_report_po_update` a left join (
                        ...
                        select a.nodo, a.tgldo, a.kodeprod, a.qty, a.nopo, a.batch_number
                            /*concat(a.nodo,a.tgldo,a.kodeprod,a.id) as batch_number*/
    ...
    public function get_po_outstanding_deltomed_old(){
        date_default_timezone_set('Asia/Jakarta'); 
        $date = date_create(date('Y-m-d H:i:s'));
        date_add($date, date_interval_create_from_date_string('-7 hours'));
        $tgl = date_format($date, 'Y-m-d H:i:s'); 
    ...
    public function get_penta_sales($token, $tahun, $bulan, $signature, $id_log)
    {
        ...
        if (isset($array_response['data'])) 
        {
            ...
            foreach ($array_response['data'] as $key) 
            {
                ...
                $dpl_do_disc_princ = $key["dpl_do_disc_princ"];
                ...
                $data = [
                    ...
                    "created_by" => $this->userid,
                    "signature" => $signature,
                    "dpl_do_disc_princ" => $dpl_do_disc_princ
                ];
    ...
    public function cek_product_penta($id_log, $bulan)
    {
        ...
        foreach ($proses->result() as $a) {
            ...
            if ($kode_produk == $kode_produk_master && $inventory_item_id == $inventory_item_id_master) 
            {
                // echo "data sama";
            }else{
    ...
    public function get_data_master_user_join_m_karyawan()
    {
        $query = "        
            select b.id, b.userid_absensi, b.username_shd
            from management_rpd.m_karyawan a 
            left join(
            select  b.id, b.userid_absensi, b.username_shd, b.active, b.kode_company
            from site.master_user b 
            )b on a.userid_pelaksana = b.id 
            where b.active = 1 and b.kode_company = 000;
        ";
    ...
    public function get_data_absensi_transaksi_join_tanggal($tahun, $bulan, $userid, $userid_absensi, $username_shd)
    {
        $query = "
            SELECT $userid as userid, $userid_absensi as userid_absensi, '$username_shd' as username_shd, DATE(a.tanggal) as tanggal, a.hari
            from site.absensi_master_tanggal a left join 
            (
                select *
                from site.absensi_transaksi a
                where year(a.tanggal) = $tahun and month(a.tanggal) = $bulan and a.userid = $userid
            )b on a.tanggal = date(b.tanggal) 
            left join (
            select *
            from mpm.`user` c 
            )c on b.userid = c.id
    ...
    public function get_data_absensi_transaksi($userid_web, $tanggal)
    {
        $query_cek = "
            select *
            from site.absensi_transaksi a 
        ...
        return $this->db->query($query_cek);
    }

    public function insert_tanggal_absensi_transaksi($data)
    {
    ...
    {   
        ...
        $query = "
            ...
            FROM    site.absensi_master_full_weekend a left join mpm.`user` b 
                        on a.userid = b.userid_absensi
            where 	year(a.tanggal) = '$tahun' and month(a.tanggal) = '$bulan'  and a.userid in (51,92,44,57)
            GROUP BY date(a.tanggal), a.userid
    ...
    public function truncate_pareto_mt($table, $tahun)
  {
    $query = "
      truncate site.$table$tahun;
    ";
    echo "<pre>";
    print_r($query);
    echo "</pre>";
  ...
  public function insert_master_outlet($tahun, $created_at)
  {
    $query = "
      insert into site.pareto_master_outlet_$tahun
      select concat(a.kode_comp, a.nocab) as site_code, 
            concat(a.kode_comp, a.kode_lang) as kode_lang, 
            a.nama_lang, a.kode_type, b.nama_type, b.sektor, b.segment,
            a.kodesalur, c.jenis, a.alamat1, '$created_at', ''
      from data$tahun.tblang a left join (
        select a.kode_type, a.nama_type, a.sektor, a.segment
        from mpm.tbl_bantu_type a 
      )b on a.kode_type = b.kode_type left join (
        select a.kode, a.jenis, a.group
  ...
  public function insert_omzet_outlet($tahun, $created_at)
  {
    $query = "
      insert into site.pareto_omzet_outlet_$tahun
      select '', a.site_code, a.outlet, a.nama_lang, a.kode_type, sum(a.tot1) as bruto, $tahun, a.bulan, '$created_at'
      from 
      (
        select concat(a.kode_comp, if(a.kode_lang is null, '', a.kode_lang)) as outlet, a.nama_lang, sum(a.tot1) as tot1, concat(a.kode_comp, a.nocab) as site_code, a.kode_type, a.bulan
        from data$tahun.fi a 
        where concat(a.kode_comp, a.nocab) in ('TGR39', 'MLG27')
        group by concat(a.kode_comp, if(a.kode_lang is null, '', a.kode_lang)), a.bulan
        union all 
        select concat(a.kode_comp, if(a.kode_lang is null, '', a.kode_lang)) as outlet, a.nama_lang, sum(a.tot1) as tot1, concat(a.kode_comp, a.nocab) as site_code, a.kode_type, a.bulan
        from data$tahun.ri a
        where concat(a.kode_comp, a.nocab) in ('TGR39', 'MLG27')
        group by concat(a.kode_comp, if(a.kode_lang is null, '', a.kode_lang)), a.bulan
      )a group by a.outlet, a.bulan
    ";
    echo "<pre>";
    print_r($query);
    echo "</pre>";
  ...
  public function insert_omzet_outlet_sub_group($tahun, $created_at)
  {
    $query = "
      insert into site.pareto_omzet_outlet_sub_group_$tahun
      select '', a.id, a.site_code, a.outlet, a.nama_outlet, a.kode_type, a.bruto, a.tahun, a.bulan, c.sub_group, '$created_at'
      from site.pareto_omzet_outlet_$tahun a inner join (
        select a.kode_type, a.nama_type, a.sektor, a.segment
        from mpm.tbl_bantu_type a 
        where a.segment = 'MT'
      )b on a.kode_type = b.kode_type inner join (
        select a.outlet, a.site_code, a.sub_group
        from site.pareto_master_outlet_mti a 
        where a.site_code is not null
      )c on a.site_code = c.site_code and a.outlet = c.outlet
    ";
  ...
  public function insert_rank($tahun, $created_at)
  {
    $query = "
      insert into site.pareto_rank_$tahun
      select 	'', a.id, if(a.sub_group is null, concat('non_group - ',a.outlet), a.sub_group) as sub_group, 
            a.nama_outlet,
            count(a.sub_group) as count_sub_group, sum(a.bruto) as bruto, $tahun, '$created_at'
      from site.pareto_omzet_outlet_sub_group_$tahun a 
      GROUP BY if(a.sub_group is null, concat('non_group - ',a.outlet), a.sub_group)
      order by sum(a.bruto) desc
      limit 100;
    ";
    echo "<pre>";
    print_r($query);
