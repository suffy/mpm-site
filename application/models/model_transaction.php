...
class Model_transaction extends CI_Model 
{
            ...
            (
                ...
                (
                    select  if(a.`open` = 0,if(concat(year(a.tglpesan),month(a.tglpesan)) = concat(year(date(now())),month(date(now()))),a.`open`,3),a.`open`) as `open`,
                            ...
                            b.bank_garansi, note_acc, d.namasupp
                    from    mpm.po a inner join mpm.user b 
                                on a.userid=b.id LEFT JOIN mpm.`user` c
                                on a.open_by = c.id left join mpm.tabsupp d 
								on a.supp = d.supp
    ...
    public function list_pelanggan_po_new($dataSegment = '')
    {
        ...
        $sql = "
                select b.kode_lang as grup_lang, b.branch_name, b.username
                from mpm.`user` b
                where b.level = 4
                GROUP BY b.kode_lang
            ";
