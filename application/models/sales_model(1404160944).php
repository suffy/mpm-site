...
class Sales_model extends CI_Model
{
    ...
    public function list_dp()
    {
...

        {
            ...
            elseif($userid == '323'){ //user miko, all jtm dan seluruh dp jawa timur (mlg, sdo, kdr, tga, mdu, blt)
                return $this->db->query('select distinct naper, nama_comp, nocab from mpm.tabcomp 
                                        where  naper!=99 and naper in(91, 28, 50, 83, 25, "g1", "b1", "p1", "p2", "t1", 27, 46, 24, 26, 23, 69, "s1", "l1")
                                        order by nama_comp');
            }
    ...
    public function print_omzet_timur($year=null,$file=null)
    {
        ...
        $sql=" 
            ...
            from
            (   
                select  naper, sub_nama, nama_comp, t1,b1,t2,b2,t3,b3,t4,b4,t5,b5,
                        t6,b6,t7,b7,t8,b8,t9,b9,t10,b10,
                        t11,b11,t12,b12,
                ...
                (
                        ...
                        select  naper,b.nama_comp as nama_comp, sub_nama,
                                sum(t1) as t1,
    ...
    public function print_omzet_barat($year=null,$file=null)
    {
        ...
        $sql=" 
            ...
            from
            (   
                select  naper, nama_comp, t1,b1,t2,b2,t3,b3,t4,b4,t5,b5,
                        t6,b6,t7,b7,t8,b8,t9,b9,t10,b10,
                        t11,b11,t12,b12,
                        total, format(sum(xb1+xb2+xb3+xb4+xb5+xb6+xb7+xb8+xb9+xb10+xb11+xb12),0, 1) jmlbulan
                from
                (
                        ...
                        select  naper,b.nama_comp as nama_comp,
                                sum(t1) as t1,
                                round(sum(b1),0) as b1,
                                sum(t2) as t2,
                                round(sum(b2),0) as b2,
    ...
    public function print_omzet($year=null,$file=null)
    {
        ...
        else if($supp=='001')
        {
            ...
            elseif ($id == '323') { //user miko, all jtm dan seluruh dp jawa timur (mlg, sdo, kdr, tga, mdu, blt)
                $wheresupp='and (kodeprod like "60%" or kodeprod like "01%" or kodeprod like "50%" or kodeprod like "70%")
                            and (nocab = 91 or nocab = 27 or nocab = 46 or nocab = 74 or nocab = 24 or nocab = 26 or nocab = 23 or nocab = 69 or nocab = "c1" or nocab = "p4" or nocab = "p5") ';
            }
    ...
    public function print_per_product($kodeprod=null,$year=null,$file=null)
    {
        ...
        switch($uv)
        {
            ...
            case 1:$unit='tot1';break;
                    ...
                    elseif ($id == '323') { //user miko, all jtm dan seluruh dp jawa timur (mlg, sdo, kdr, tga, mdu, blt)
                        $tambahan='and (nocab = 91 or nocab = 27 or nocab = 46 or nocab = 74 or nocab = 24 or nocab = 26 or nocab = 23 or nocab = 69 or nocab = "c1" or nocab = "p4" or nocab = "p5")';
                    }
    ...
    public function sales_omzet_barat($limit=null,$offset=null,$year=null,$retur=null)
    {
        ...
        else
        {
                ...
                $sql="
                        ...
                        inner join `mpm`.`tabcomp` `b` USING (`nocab`) where b.wilayah = 1
                        group by `naper`
                        order by `naper`
            ";
             $query = $this->db->query($sql);
