...
class M_assets extends CI_Model
{
    ...
    public function get_pr($id = ''){
        ...
        $sql = "
                ...
                )b on a.no_po = b.no_po and a.id_barang = b.id_ref_barang
    ...
    public function get_pr_asset($id = ''){
        ...
        $sql = "
                ...
                WHERE a.no_po not in (SELECT a.no_po from mpm.asset a
                WHERE a.no_po is not null and no_po not in ('Hanya Mutasi'))
                OR a.id_barang not in (SELECT a.id_ref_barang from mpm.asset a
                WHERE a.id_ref_barang is not null)
                GROUP BY a.no_po, a.id_barang
                ORDER BY a.id = 15 desc, a.id desc
    ...
    public function get_history_penyerahan_asset($no_po, $id_ref_barang)
    {
        $sql = "
            select a.*, b.username from site.t_asset_penyerahan_asset a
            left join mpm.`user` b on a.userid_penerima = b.id
            where a.no_po = '$no_po' and a.id_ref_barang = '$id_ref_barang'
        ";
