	...
	function viewPiutang($data){
		...
		$sql="
			select  kode_comp,grup_lang,group_descr grup_nama,saldoawal,debit,kredit,
					...
					r1 ,r2 ,r3 ,r4,r5, total, '$tanggal' as tanggal
			from
			(
					select  z.group_id grup_lang,z.group_descr,
							...
							current,duedate,r1,r2,r3,r4,r5,
							current+duedate+r1+r2+r3+r4+r5 total
					from
					(   
					...
					(
						select  group_id, group_descr,
								...
								SUM(IF(days_past_due > 1, amount_due, 0))r5
						FROM
						(
							SELECT  group_id, group_descr, 
		...
		if ($key=='1') {
			...
			(
				select 	right(trim(no_polisi),8) nodokjdi,ref nodokacu,
						...
						SUM(IF(days_past_due > 1, amount_due, 0)) nilai
				FROM
				(
						SELECT 	no_polisi,ref,tgl_tempo,tanggal,no_sales,group_id, group_descr, 
								datediff('$end',tgl_tempo) days_past_due, 
								sum(saldo) amount_due 
						FROM
						(
							SELECT 	no_polisi,ref,tanggal,no_sales ,a.customerid ,
									tgl_tempo ,( IF ( a.retur = 1, a.dokument *- 1, a.dokument )) - IF (isnull(kredit), 0, kredit) saldo
							FROM 	dbsls.t_ar_ink_master a 
							LEFT JOIN 
							(
								SELECT 	no_sales,customerid, sum( bayar_transfer + bayar_giro + bayar_tunai ) kredit 
