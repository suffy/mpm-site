...
class M_dc extends CI_Model
{
    ...
    public function insert_temp_dc_do($nodo, $created_at, $userid, $signature, $kode)
    {
        ...
        $sql = "
            insert into site.temp_dc_do
            select '', a.id_do , '$kode', a.nodo, a.tgldo, a.nopo, a.tglpo, a.tipe, a.kodeprod, a.namaprod, a.banyak, a.company, a.kode_alamat, b.alamat, a.alamat_kirim, '$created_at', $userid, '$signature',0
            from
            (
                select a.id as id_do, a.nodo, date(a.tgldo) as tgldo, a.nopo, b.tglpo, b.tipe, a.kodeprod, a.namaprod, a.banyak, b.company, b.kode_alamat, b.alamat, b.alamat_kirim
                from db_po.t_do_us a INNER JOIN
                (
                    select a.nopo, a.tglpo, a.tipe, a.company, a.kode_alamat, a.alamat, a.alamat_kirim
                    from mpm.po a 
    ...
    public function get_data_row_keluar($signature = '')
    {
        if ($signature == '') {
            ...
            $signature_params = "and a.signature = '$signature'";
        }
    ...
    public function get_data_row_keluar_by_kodeprod($signature = '')
    {
        ...
        $sql = "
            ...
            where a.deleted <> 1 $signature_params
            ORDER BY a.kodeprod
        ";
    ...
    {
        $query = "            
            select a.id, a.kode, a.link, a.signature, b.signature_dc, b.created_at, b.tanggal_kirim, b.file_1, b.file_2, b.file_3, b.file_4, b.tanggal_tiba, DATEDIFF(b.tanggal_tiba,b.tanggal_kirim) as leadtime
            from 
            (
                select 	a.id, a.id_do, a.kode, 
    ...
    public function generate_keluar()
    {
        ...
        $tahun_now = date('Y');
        // $bulan_now = '02';
        // echo "bulan_now : " . $bulan_now;
        // echo "<br>";
        // echo "tahun_now : " . $tahun_now;

        $cek_jumlah = $this->db->get("site.t_dc_do_keluar")->num_rows();
        if (!$cek_jumlah) {
            $reserve_nomor_params = "MPM_DO/" . date('Y') . $bulan_now . "-001";
        } else {
            // echo "ada";
            $sql = "
                select right(a.kode,3) as urut
                from site.t_dc_do_keluar a
                where MONTH(a.created_at) = $bulan_now and YEAR(a.created_at) = $tahun_now
                GROUP BY a.kode
                ORDER BY right(a.kode,3) desc
                limit 1
            ";
            ...
            $reserve_nomor = $proses->urut + 1;
            if (strlen($reserve_nomor) === 1) {
                $reserve_nomor_params = "MPM_DO/" . date('Y') . $bulan_now . "-00" . $reserve_nomor;
            } else if (strlen($reserve_nomor) === 2) {
                $reserve_nomor_params = "MPM_DO/" . date('Y') . $bulan_now . "-0" . $reserve_nomor;
            } else {
                $reserve_nomor_params = "MPM_DO/" . date('Y') . $bulan_now . "-" . $reserve_nomor;
            }
            // echo "reserve_nomor_params : ".$reserve_nomor_params;
        }
    ...
    public function get_vendor_dc(){
        $query = "
        select a.site_code, a.nama_comp, a.vendor, a.alamat_kirim, a.email, a.no_wa, a.created_by, a.created_at, b.username
        from site.map_dc_po a LEFT JOIN 
        (
    ...
    public function get_vendor($kode_alamat){
        // echo $kode_alamat;
