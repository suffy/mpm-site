...
class Model_omzet extends CI_Model 
{
	...
	/* ---------DEFINISI VARIABEL----------------- */
		...
		//echo "supplier di model : ".$supp;
            ...
            {                
                if ($id == '318') { //jika user dimas, lihat all jkt & karawanga
...
(
	...
	(
		select concat(kode_comp, nocab) as kode,naper,kode_comp, nama_comp, sub,urutan,`status`,status_cluster
		from mpm.tbl_tabcomp_new
		where `status` = 1 and ".$wilayah."".$tambahDP."
		GROUP BY concat(kode_comp, nocab)
	)b on a.kode = b.kode

	union ALL

	select 	a.nocab, '', b.sub, b.nama_comp,
				sum(t1),sum(b1),sum(t2),sum(b2),sum(t3),sum(b3),sum(t4),sum(b4),sum(t5),sum(b5),sum(t6),sum(b6),sum(t7),sum(b7),sum(t8),sum(b8),
				sum(t9),sum(b9),sum(t10),sum(b10),sum(t11),sum(b11),sum(t12),sum(b12),  
				".$id.", total, rata,
				".'"'.$supp.'"'.",".$tgl_created.",".$year.", b.urutan
from
(
		SELECT  a.nocab, a.kode_comp, b.sub, b.nama_comp,
					sum(t1) t1,sum(b1) b1,sum(t2) t2,sum(b2) b2,sum(t3) t3,sum(b3) b3,sum(t4) t4,sum(b4) b4,sum(t5) t5,sum(b5) b5,
					sum(t6) t6,sum(b6) b6,sum(t7) t7,sum(b7) b7,sum(t8) t8,sum(b8) b8,
					sum(t9) t9,sum(b9) b9,sum(t10) t10,sum(b10) b10,sum(t11) t11,sum(b11) b11,sum(t12) t12,sum(b12) b12,  
					".$id.", format(total,0) total,	format(totalx/jmlbulan,0) rata,
					".'"'.$supp.'"'.",".$tgl_created.",".$year.", urutan
			FROM
			(
