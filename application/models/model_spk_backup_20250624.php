...
class Model_spk extends CI_Model 
{
    ...
    {
        // $query = "
        //     select a.tgldo, b.username, a.created_at, unit_do
        //     from 
        //     (
        //         select a.tgldo, sum(a.qty) as unit_do, a.created_at, a.created_by
        //         from site.t_do_deltomed a
        //         GROUP BY a.tgldo
        //         ORDER BY tgldo desc
        //     )a LEFT JOIN site.master_user b 
        //         on a.created_by = b.id
        //     limit 100
        // ";
    ...
    public function get_do_deltomed_by_nopo($nopo)
    {
        $query = "
            select 	a.nodo, str_to_date(a.tgldo,'%d/%m/%Y') as tgldo, a.kodedp, a.company, 
                    ...
                    a.batch_number, c.tanggal_terima, c.id, b.kodeprod, c.qty as qty_terima
            from site.t_do_deltomed a 
            left join (
    ...
    public function update_po_detail_actual_po_bulan($data, $actual_po)
    {
        foreach ($actual_po->result() as $key) {
            $update = [
                'actual_po_bulan_ini' => $key->banyak
            ];
    ...
    {
        // $query = "
        //     select a.nodo, a.tgldo, a.nopo, count(DISTINCT(a.nopo)) as count_nopo, count(DISTINCT(a.nodo)) as count_nodo
        //     from db_po.t_do_us a 
        //     group by a.tgldo
        // ";
    ...
    public function history_helpdesk($form_search)
    {
        if ($form_search != null) {
            ...
            if ($status == 999) {
                $param_status = "and b.`status` in (0,1,2)";
            } else {
                $param_status = "and b.`status` in ($status)";
            }
            
            $param = "WHERE (a.created_at BETWEEN '$from 23:59:00' and '$to 23:59:00') $param_status";
        }else{
            $param_status = "WHERE b.`status` in (0,1,2)";
            $param = "";
        }
    ...
    public function history_helpdesk_by_userid($userid, $form_search)
    {
        if ($form_search != null) {
            $from = $form_search['from'];
            $to = $form_search['to'];
