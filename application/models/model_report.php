...
class Model_report extends CI_Model 
{
	...
	/* ---------DEFINISI VARIABEL----------------- */
		...
		select 	supp, namasupp, 
				SUM(total_po) as total_po, SUM(actual_po) as actual_po, SUM(approve) as approve, SUM(pending) as pending, 
				format(SUM(total_values_all_po),0) as all_value,
				format(SUM(total_actual_po),0) as actual_value, 
				format(SUM(total_values_po_approve),0) as approve_value,
				format(SUM(total_values_po_pending),0) as pending_value
		FROM
		(
			select 	a.supp as supp, 
					namasupp, 
					COUNT(*) as total_po,
					'' as actual_po,
					'' as approve,
					'' as pending,
					SUM(total_value) as total_values_all_po,
					'' as total_actual_po,
					'' as total_values_po_approve,
					'' as total_values_po_pending
			FROM(
					...
					(
						...
						from	 mpm.po a INNER JOIN mpm.tabsupp b
											on a.supp = b.SUPP
						where  a.created like '$year-$month%' and a.deleted = 0
						order by id desc
			...
			select 	a.supp as supp, namasupp, 
					'' as total_po,
					COUNT(*) as actual_po,				
					'' as approve,
					'' as pending,
					'' as total_values_all_po,
					SUM(actual_po) as total_actual_po,
					'' as total_values_po_approve,
					'' as total_values_po_pending
			FROM(
						
					select	a.id, a.supp, SUM(banyak*harga) as `actual_po`, namasupp, company, nopo, tglpo, created, a.deleted
					from
					(
						select a.id id, a.supp supp,b.NAMASUPP namasupp, company, nopo, tglpo, tglpesan, a.created, a.modified, tipe, `open`, open_date, open_by, a.deleted
						from	 mpm.po a INNER JOIN mpm.tabsupp b
											on a.supp = b.SUPP
						where  	a.deleted = 0 and month(tglpo) = $month and
								year(tglpo) = $year
						order by id desc
					)a LEFT JOIN mpm.po_detail b
							on a.id = b.id_ref
			...
			select 	a.supp, namasupp, 
					...
					'', 
					'',
					SUM(total_value) as total_values, ''
			FROM(
					...
					(
						...
						from	 mpm.po a INNER JOIN mpm.tabsupp b
											on a.supp = b.SUPP
						where  a.created like '$year-$month%'  and `open` = '1' and a.deleted = 0
						order by id desc
					)a LEFT JOIN mpm.po_detail b
							on a.id = b.id_ref
					where b.deleted = 0
			...
			select 		a.supp, 
						...
						'',
						'',
						'', SUM(total_value) as total_values_po_pending
			FROM(
					...
					(
						...
						from	 mpm.po a INNER JOIN mpm.tabsupp b
											on a.supp = b.SUPP
						where  a.created like '$year-$month%'  and `open` = '0' and a.deleted = 0
						order by id desc
					)a LEFT JOIN mpm.po_detail b
							on a.id = b.id_ref
					where b.deleted = 0
		...
		//mendefinisikan variabel
			...
			*/
				...
				*/
				...
				$this->db->where('po_detail.deleted = 0');
		        ...
		        $this->db->join('po_detail', 'po_detail.id_ref = po.id','inner');
		        $this->db->join('tabsupp', 'po.supp = tabsupp.supp','left');
		        $this->db->select('po.nopo, po.id, po.created, po.tglpo,po.company, tabsupp.namasupp, po.nodo,po.tgldo');
				$hasil = $this->db->get('mpm.po');

	   		}elseif ($kategori == 'actual') {

	   			//echo "tampil po actual (po yang dijalankan)";

				$this->db->group_by("po.id"); 
				$this->db->where('po_detail.deleted = 0');
				$this->db->where('po.deleted = 0');
				$this->db->where('po.supp = '.$dataSegment['supplier']);
				$this->db->where('month(po.tglpo) = '.'"'.$dataSegment['bulan'].'"');
				$this->db->where('year(po.tglpo) = '.'"'.$dataSegment['tahun'].'"');
		        //$this->db->where('po.created like '.'"'.$dataSegment['tahun'].'-'.$dataSegment['bulan'].'%'.'"');
		        $this->db->join('po_detail', 'po_detail.id_ref = po.id','inner');
		        $this->db->join('tabsupp', 'po.supp = tabsupp.supp','left');
		        $this->db->select('po.nopo, po.id, po.created, po.tglpo, po.company, tabsupp.namasupp, po.nodo,po.tgldo');
