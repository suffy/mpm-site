...
class Model_sales_omzet extends CI_Model
{
    ...
    public function get_kodeprod($supp){
        ...
        if ($supp === 'XXX') {
            ...
            $sql = $this->db->query("select kodeprod from mpm.tabprod where supp <> 'BSP' $session_supp and (grup != 'gimmick' or grup is null)")->result();            
        }else{

            // $sql = "select kodeprod from mpm.tabprod where supp in ($supp) $session_supp";
            // $proses = $this->db->query($sql)->result();
            
            $sql = $this->db->query("select kodeprod from mpm.tabprod where supp in ($supp) $session_supp and (grup != 'gimmick' or grup is null)")->result();
        }
    ...
    public function sales_per_product($data){
        ...
        if ($custom_outlet == '1') { // jika outlet default
            // echo "1";
            $custom_outlet_x = "";
            ...
            $params_sektor = "e.sektor";

        }else{ // jika outlet deltomed
            // echo "2";
            $custom_outlet_x = "
            inner JOIN 
            (
        ...
        if($kondisi_class == 2 && $breakdown_kodesalur == '1'){
            ...
            (
                ...
                FROM
                (
                    select concat(kode_comp, nocab) as kode, CONCAT(KODE_COMP,kode_lang) as outlet, CONCAT(KODE_COMP,kode_lang) as outletx, `$qty`, `tot1`, kodeprod, bulan, kodesalur, kode_type, concat(kodesales,nocab) as kode_sales, kodesales
                    from `data$tahun`.`fi`
                    where `kodeprod` in ($kodeprod) and `nodokjdi` <> 'XXXXXX' ".$wilayah."
                    union all
                    select concat(kode_comp, nocab) as kode, null, CONCAT(KODE_COMP,kode_lang) as outletx, `$qty`, `tot1`, kodeprod, bulan, kodesalur, kode_type, concat(kodesales,nocab) as kode_sales, kodesales
                    from `data$tahun`.`ri`
                    where `kodeprod` in ($kodeprod) and `nodokjdi` <> 'XXXXXX' ".$wilayah."
                )a $custom_outlet_x
                LEFT JOIN
        ...
        $sql="
                ...
                select  a.kode,b.branch_name,b.nama_comp, b.sub,
                        a.kodeprod,a.namaprod,a.grup, c.nama_group,a.kodesalur,d.namasalur,d.groupsalur, a.kode_type, e.nama_type, $params_sektor as sektor, e.segment,a.subgroup, f.nama_sub_group, a.kodesales, g.namasales,
        ...
        public function outlet($data,$kodeprod=null){
            ...
            foreach($naper as $dp){
                ...
                $query2 = "
                    ...
                    a.NOCAB, '' as kodelang_copy, '' as id_provinsi, '' as nama_provinsi, '' as id_kota, '' as nama_kota, '' as id_kecamatan,
                    '' as nama_kecamatan, '' as id_kelurahan, '' as nama_kelurahan, '' as credit_limit, '' as tipe_bayar,'' as phone,
                    '' as last_updated, '' as status_payment, '' as CUSTID, '' as COMPID, '' as LATITUDE, '' as LONGITUDE, '' as FOTO_DISP, '' as FOTO_TOKO
    ...
    public function get_list_raw()
    {
        ...
        if ($supp == 000) {
            $sql = "
                ...
                WHERE userid = $id and b.status = 1 and b.supp != 901 and b.keterangan != 'harian' 
            ...
            $sql = "
                ...
                INNER JOIN db_raw.t_list_raw b on a.region = b.region
                LEFT JOIN mpm.tabsupp c on b.supp = c.supp 
                WHERE userid = $id and b.status = 1 and b.supp = $supp and b.keterangan != 'harian'
                order by b.urutan asc, b.id desc
            ";
        }
