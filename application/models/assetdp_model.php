<?php
class Assetdp_model extends CI_Model
{
    var $total_query='';
    var $output_table = '';
    var $output_print = '';
    ...
    {
        $sql='select a.*,b.id idwilayah,b.wilayah from mpm.assetdp a inner join mpm.assetkota b on a.idkota=b.id where a.id=?';
        return $this->db->query($sql,array($id));
    }
    public function getGrupasset($id=null)
    {
        $sql='select * from mpm.grupassetdp where id=?';
        return $this->db->query($sql,array($id));
    }
    public function getGrupassetcombo()
    {
        $sql='select id,namagrup from mpm.grupassetdp';
        return $this->db->query($sql);
    }
    public function getKotacombo()
    {
        $sql='select id,wilayah from mpm.assetkota where userid='.$this->session->userdata('id').' order by wilayah';
        return $this->db->query($sql);
    }
    public function delete_asset($id=null)
    {
        $sql='delete from mpm.assetdp where id=?';
        $this->db->trans_begin();
        ...
        redirect('assetdp/show_asset/');
    }
     public function delete_grupasset($id=null)
    {
        $sql='delete from mpm.grupassetdp where id=?';
        $this->db->trans_begin();
        $this->db->query($sql,array($id));
        $this->db->trans_complete();
        if($this->db->trans_status()===FALSE)
        ...
        redirect('assetdp/show_grupasset/');
    }
    ...
    public function update_grupasset($id)
    {
        ...
        $where='id='.$id;
        $this->db->trans_begin();
        $st=$this->db->update_string('mpm.grupassetdp',$post,$where);
        $this->db->query($st);
        $this->db->trans_complete();
        if($this->db->trans_status()===FALSE)
        {
    ...
    private function print_asset_excel($tanggal=null,$grupid=0)
    {
        if ($grupid!='')
        {
            ...
            $sql="select id,grupid,kode,namagrup,jumlah,namabarang,untuk,tglperol,tglbeli,
                persen,nilai as np
                ,golongan
                ,format(bln,2) as perbulan
                ,format(if(total>=nilai,nilai*1,total),2) as totalakm
                ,format(if(nilai-total<0,0,(nilai-total)),2) as nilaibuku
                ,if(nj=0 or nj='','AKTIF','JUAL') as status
                from(
                select a.created_by,a.id,kode,NJ,grupid,b.namagrup,namabarang,jumlah,untuk,tglperol,tgljual, tglperol as tglbeli,
                ...
                from mpm.assetdp a
                left join mpm.grupassetdp b on a.grupid=b.id order by id desc
                )a
                ...
                and period_diff(".$tanggal.",date_format(tglbeli,'%Y%m'))>=0 and created_by=".$this->session->userdata('id')."
                order by namagrup";
            ...
            $sql="select id,grupid,kode,namagrup,jumlah,namabarang,untuk,tglperol,tglbeli,
                persen,nilai as np
                ,golongan
                ,format(bln,2,'id_ID') as perbulan
                ,format(if(total>=nilai,nilai*1,total),2,'id_ID') as totalakm
                ,format(if(nilai-total<0,0,(nilai-total)),2,'id_ID') as nilaibuku
                ,if(nj=0 or nj='','AKTIF','JUAL') as status
                from(
                select a.created_by,a.id,kode,NJ,grupid,b.namagrup,namabarang,jumlah,untuk,date_format(tglperol,'%d/%m/%Y') as tglperol,tgljual, tglperol as tglbeli,
                format(gol*100,2) as persen,
                np as nilai,
                if(gol='0.25','I',if(gol='0.125','II',if(gol='0.0625','III','B'))) as golongan,
                (gol*np/12) as bln,
                ...
                where period_diff(".$tanggal.",if(date_format(tgljual,'%Y%m')=197001,date_format(now(),'%Y%m'),
                date_format(tgljual,'%Y%m')-1))<=0
                and period_diff(".$tanggal.",date_format(tglbeli,'%Y%m'))>=0 and a.created_by=".$this->session->userdata('id')."
                order by namagrup";
            $query=$this->db->query($sql);
            return to_excel($query);
        }
    ...
    private function print_asset_excel_nb($tanggal=null,$grupid=0)
    {
        ...
        */
        ...
        if ($grupid!='')
        {
            ...
            $sql="
                select *
                from(select id,grupid,kode,namagrup,jumlah,namabarang,untuk,tglperol,tglbeli,
                persen,nilai as np
                ,golongan
                ,format(bln,2,'id_ID') as perbulan
                ,format(if(total>=nilai,nilai*1,total),2,'id_ID') as totalakm
    ...
    private function print_asset_jual_excel()
    {
        $sql="select id,grupid,kode,namagrup,jumlah,namabarang,untuk,tglperol,persen,format(nilai,2) as np,format(nj,2) as nj,tgljual
            ...
            from(
            select a.created_by,a.id,kode,grupid,b.namagrup,namabarang,jumlah,untuk,tglperol,
            ...
            where nj!=0 and created_by=".$this->session->userdata('id');
          $query=$this->db->query($sql);
           return to_excel($query);
    }
    ...
    public function print_asset()
    {
        ...
        $post['wilayah']=$this->input->post('wilayah');
        //$post['tglsusut']='201108';
        ...
        switch($format)
        {
            case 'PDF':
                ...
                if($grupid!=null)
                {
                    ...
                    //echo 'uyk';
                    $post['grupid']=$code;
                    $xml = simplexml_load_file("assets/report/assetdp/report_penyusutan.jrxml");
                   
                }
                else
                {
                    //echo 'uykasdas';
                    $xml = simplexml_load_file("assets/report/assetdp/report_penyusutan_all.jrxml");
                }
                ...
                @$this->phpjasperxml->xml_dismantle($xml);
                @$this->phpjasperxml->transferDBtoArray($server,$user,$pass,$db);
                @$this->phpjasperxml->outpage("D","asset.pdf");
    ...
    public function print_asset_jual()
    {
        ...
        switch($format)
        {
            case 'PDF':
                ...
                $db='mpm';
                $this->load->library('PHPJasperXML');
                $xml = simplexml_load_file("assets/report/assetdp/report_penyusutan_jual.jrxml");
                $post['userid']=$this->session->userdata('id');
                @$this->phpjasperxml->debugsql=false;
                @$this->phpjasperxml->arrayParameter=$post;
                @$this->phpjasperxml->xml_dismantle($xml);
    ...
    public function show_asset($limit=null,$offset=null,$keyword=null)
    {
        ...
        if ($userid == '316') {
            ...
            select  a.id,
                    ...
                    jumlah,
                    untuk,
                    date_format(tglperol,'%d-%M-%Y') as tglperol,
                    gol,
                    np 
            from    mpm.assetdp a
                    left join mpm.grupassetdp b on a.grupid=b.id 
                    left join mpm.assetkota c on a.idkota=c.id
            where   namabarang like '%".$keyword."%' and a.created_by='312' 
            order by b.namagrup,kode
            ";
        } else {
            $sql="
            select  a.id,
            ...
            from    mpm.assetdp a
                    left join mpm.grupassetdp b on a.grupid=b.id 
                    left join mpm.assetkota c on a.idkota=c.id
            where   namabarang like '%".$keyword."%' and a.created_by='".$userid."' 
            order by b.namagrup,kode
        ";
        ...
        if($query->num_rows()>0)
        {
            ...
            $this->table->set_heading('Kode','Nama Barang','Grup', 'Jumlah','Untuk','Wilayah','Tgl Perol','Gol','%','Nilai Perolehan','Status','EDIT','DELETE');
            foreach ($query->result() as $value)
            {
                ...
                $this->table->add_row(
                        ...
                        ,'<div div style="text-align:center">'.anchor('assetdp/edit_asset/'.$value->id,img($image_properties_edit)).'</div>'
                        ...
                        //,$image_email_po
                        ,'<div div style="text-align:center">'.anchor('assetdp/delete_asset/'.$value->id,img($image_properties_del),$js).'</div>'
                       );
            }
   ...
   public function show_grupasset($limit=null,$offset=null)
    {
        $sql="select * from mpm.grupassetdp";
        $query_sum = $this->db->query($sql);
        $this->total_query = $query_sum->num_rows();
        $sql2= $sql.' limit ? offset ?';
        $query = $this->db->query($sql2,array($limit,$offset));
        if($query->num_rows()>0)
        {
            ...
            foreach ($query->result() as $value)
            {
                ...
                $this->table->add_row(
                        $value->namagrup
                       ,$value->deskripsi
                       ,'<div div style="text-align:center">'.anchor('assetdp/edit_grupasset/'.$value->id,img($image_properties_edit)).'</div>'
                       ,'<div div style="text-align:center">'.anchor('assetdp/delete_grupasset/'.$value->id,img($image_properties_del),$js).'</div>'
                       );
            }
    ...
    public function save_asset($str_id='')
    {
        ...
        $this->db->trans_begin();
        $this->db->insert('mpm.assetdp',$post);
        $this->db->trans_complete();
        if($this->db->trans_status()===FALSE)
        {
            $this->db->trans_rollback();
    ...
    public function save_grupasset($str_id='')
    {
        ...
        $this->db->trans_begin();
        $this->db->insert('mpm.grupassetdp',$post);
        $this->db->trans_complete();
        if($this->db->trans_status()===FALSE)
        {
            $this->db->trans_rollback();
