...
class Model_inventory extends CI_Model
{
    ...
    public function stock_akhir_doi($data, $created_at)
    {
        ...
        where a.kodeprod in ($kodeprod) $wilayahx and right(a.kode,2) != 'M2'
        ";
    ...
    public function laporan_po($data)
    {
        ...
        select  if(a.company like 'PT. MPI%', 'MPI', if(a.company like 'PT. PENTA VALENT TBK%', 'PT. PENTA VALENT TBK', e.branch_name)) as branch_name,
    ...
    public function get_kode_alamat()
    {
        ...
        }elseif ($this->session->userdata('username') == 'janny' || $this->session->userdata('username') == 'celvin' || $this->session->userdata('username') == 'eta') {
            $params = "where b.kode_alamat is not null and a.username like 'MPI%'";
        }else{
    ...
    public function get_do($supp, $id)
    {
        ...
        if ($supp == '001') {
            ...
            (
                select a.id, a.kodeprod, a.status_terima, a.tanggal_terima, a.deleted
                from mpm.po_detail a
                where a.id_ref = $id and a.deleted = 0
            )c on b.kodeprod = c.kodeprod
            where a.nopo = '$nopo' and c.deleted = 0
            ";
            ...
            (
                select a.id, a.kodeprod, a.status_terima, a.tanggal_terima, a.banyak, a.banyak_karton, a.deleted
                from mpm.po_detail a
                where a.id_ref = $id and a.deleted = 0
            )c on b.kodeprod = c.kodeprod
            where a.id_po = $id and c.deleted = 0
            ";
            $proses = $this->db->query($sql)->result();
            if ($proses) {
                return $proses;
    ...
    public function po_outstanding_all($data)
    {
        // var_dump($data);
        // die;
        $supp = $data['supp'];
        ...
        if ($hasil_traffic == 1) {
            ...
            $insert_print = "
            insert into db_po.t_po_outstanding_all_print
            select  a.id, a.branch_name, a.nama_comp, a.company, a.tglpo,month(a.tglpo) as bulan_po, a.nopo, a.tipe, a.kodeprod, a.namaprod, 
                    a.qty_po, a.kode_alamat, 
                    '$tgl', $id, a.supp
            from    db_po.t_po_outstanding_all a
            where   $kode_alamat_params (date(a.tglpo) between '$periode1' and '$periode2') and a.supp = '$supp'";
            $this->db->query($insert_print);
            ...
            $sql = "
            select * from db_po.t_po_outstanding_all a
            where $kode_alamat_params (date(a.tglpo) between '$periode1' and '$periode2') and a.supp = '$supp'";
