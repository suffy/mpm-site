...
class Model_master_data extends CI_Model 
{
  ...
  public function insert_master_outlet($tahun)
  {
    $query = "
  ...
  public function insert_omzet_outlet($tahun)
  {
    $query = "
      insert into site.pareto_omzet_outlet_$tahun
      select a.site_code, a.outlet, a.nama_lang, a.kode_type, sum(a.tot1) as bruto, $tahun
      from 
      (
        select concat(a.kode_comp, if(a.kode_lang is null, '', a.kode_lang)) as outlet, a.nama_lang, sum(a.tot1) as tot1, concat(a.kode_comp, a.nocab) as site_code, a.kode_type
        from data$tahun.fi a 
        group by concat(a.kode_comp, if(a.kode_lang is null, '', a.kode_lang))
        union all 
        select concat(a.kode_comp, if(a.kode_lang is null, '', a.kode_lang)) as outlet, a.nama_lang, sum(a.tot1) as tot1, concat(a.kode_comp, a.nocab) as site_code, a.kode_type
        from data$tahun.ri a
        group by concat(a.kode_comp, if(a.kode_lang is null, '', a.kode_lang))
  ...
  public function insert_omzet_outlet_subgroup($tahun)
  {
    $query = "
      insert into site.pareto_omzet_outlet_sub_group_$tahun
      select a.*, c.sub_group
      from site.pareto_omzet_outlet_$tahun a inner join (
        select a.kode_type, a.nama_type, a.sektor, a.segment
        from mpm.tbl_bantu_type a 
        where a.segment = 'MT'
      )b on a.kode_type = b.kode_type inner join (
        select a.kode_outlet, a.site_code, a.sub_group
        from test.bantu_mapping_outlet_mti a 
      )c on a.site_code = c.site_code and a.outlet = c.kode_outlet;
    ";
    echo "<pre>";
    print_r($query);
    echo "</pre>";
  ...
  public function insert_rank($tahun)
  {
    $query = "
      insert into site.pareto_rank_$tahun
      select 	'', if(a.sub_group is null, concat('non_group - ',a.outlet), a.sub_group) as sub_group, 
            a.nama_outlet,
            count(a.sub_group) as count_sub_group, sum(a.bruto) as bruto, $tahun
