        ...
        switch($state)
        {
            case 'bsp':
            {
                 ...
                 $sql="select nama_comp cabang,nama_type tipe,deskripsi produk,sum(banyak/isisatuan) unit ,sum(jumlah) value,substr(tgldokjdi,6,2) bulan from 
                       bsp.bspsales".$tahun." a inner join  bsp.tabprod b on a.deskripsi=b.namaprod 
            ...
            case 'dp':
            {
                ...
                $sql2="select c.namaprod ,nama_comp dp,c.grupprod,(banyak) unit,(tot1) value,blndok month from data".$tahun.".fi a 
                        inner join mpm.tabcomp b using(nocab) 
                        inner join mpm.tabprod c using(kodeprod)
                         ".$kondisi." group by namaprod,nama_comp,c.grupprod,unit,value,month
                        union all
                        select c.namaprod ,nama_comp dp,c.grupprod,(banyak) unit,(tot1) value,blndok month from data".$tahun.".ri a 
                        inner join mpm.tabcomp b using(nocab) 
                        inner join mpm.tabprod c using(kodeprod)
                        where ".$kondisi." group by namaprod,nama_comp,c.grupprod,unit,value,month"; 
