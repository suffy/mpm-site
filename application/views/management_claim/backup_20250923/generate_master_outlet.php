    ...
    <div class="card-body">
        ...
        <div class="row mt-5">
            ...
            <div class="col-md-5 d-flex flex-row">
                ...
                <select name="site_code" id="site_code" class="form-control custom-input" required>
                    <?php
                        foreach ($site_code->result() as $a) { ?>
                            <option value="<?= $a->site_code ?>"><?= $a->branch_name." - ".$a->nama_comp ?></option>
                        <?php
                        }
                    ?>
                </select>
        ...
        <div class="row mt-1">
            <div class="col-md-2">
                <label for="site_code" class="form-label">Sales on</label>
            </div>
            <div class="col-md-5 d-flex flex-row">
            ...
            ?>
                ...
                <input type="month" class="form-control" name="month" value="<?= $params_month ?>" required>
            </div>
        </div>

        <div class="row mt-1">
            <div class="col-md-2">
                <label for="site_code" class="form-label">Tipe Outlet</label>
            </div>
            <div class="col-md-5 d-flex flex-row">
                <select name="kode_type" id="kode_type" class="form-control custom-input" required>
                </select>
            </div>
        </div>
        ...
        <div class="row mt-3">
            ...
            <div class="col-md-5 d-flex flex-row gap-1">
                <button type="submit" class="btn btn-submit-red" style="height: 45px;">retrive outlet</button> 
                <a href="<?= base_url('management_claim/master_outlet') ?>" class="btn btn-submit-black" style="height: 45px;padding: 10px">outlet registered</a>
            </div>
        </div>
    </div>
                        ...
                        <table id="generate-outlet" style="width: 100%">
                            <thead>
                                <tr>   
                                    ...
                                    <th style="text-align: center; vertical-align: middle;">Nama Outlet</th>  
                                </tr>
                            </thead>
                                            ...
                                            <td style="text-align: left;">
                                                <div class="d-flex flex-row gap-5">
                                                    ...
                                                    <div>
                                                        <?php 
                                                            if ($a->status_register) { ?>
                                                                <span class="btn pending-rilis-po">already registered</span>
                                                            <?php
                                                            }else{ ?>
                                                                <span class="btn pending-scm">available</span>
                                                            <?php
                                                            }
                                                        ?>
                                                    </div>
...
<script>
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('management_claim/master_type_outlet') ?>',
        success: function(result) {
            $("select[name = kode_type]").html(result);
        }
    });
