                            ...
                            <table class="table table-sm table-striped" id="biop-table">
                                ...
                                <tbody>
                                    ...
                                    foreach ($get_data_biop_grouped as $row):
                                        $total = $row->tol + $row->parkir + $row->bbm_rp + $row->makan + $row->jamuan + $row->meeting + $row->perjalanan_dinas + $row->service_kendaraan + $row->stationery + $row->lain ;
                                        $total_tol += $row->tol;
                                        ...
                                        $total_stationery += $row->stationery;
                                        $total_lain += $row->lain;
                                        $grand_total += $total;
                                    ?>
                                    <tr>
                                        ...
                                        <td class="text-start"><?= is_array($row->keterangan_biaya) ? implode(', ', $row->keterangan_biaya) : $row->keterangan_biaya ?></td>
                                        ...
                                        <td><?= $row->stationery ? number_format($row->stationery) : '-' ?></td>
                                ...
                                <tfoot>
                                    <tr class="table-active">
                                        ...
                                        <td><strong><?= number_format($total_stationery) ?></strong></td>
                                    ...
                                    foreach ($get_data_biop as $biop) :
                                        ...
                                        <tr>
                                            ...
                                            <!-- <td><?= $biop->jamuan_tempat; ?></td> -->
                                             <td><?= json_encode($biop->jamuan_tempat) ?></td>
                                            <td><?= $biop->jamuan_alamat; ?></td>
                                            <td><?= $biop->jamuan_jenis; ?></td>
                                            <td><?= $biop->biaya_approved; ?></td>
                                        </tr>
                                    <?php }
                                    endforeach; ?>
                                </tbody>
            ...
            <div class="tab-pane fade" id="nav-pengeluaran" role="tabpanel" aria-labelledby="nav-pengeluaran-tab">
                <div class="card" id="pengeluaran-content">
                    <div class="card-body">
                        ...
                        <!-- <div class="table-responsive tabel-pengeluaran-container">
                            <table class="table table-sm table-bordered">
                                ...
                                <tbody>
                                    ...
                                    // mapping judul view -> key array pada controller
                                    $list_kategori = [
                                        'Biaya BBM/Tol/Parkir' => 'biaya_bbm',
                                        'Biaya Jamuan' => 'biaya_jamuan',
                                        'Biaya Meeting' => 'biaya_meeting',
                                        'Biaya Perjalanan Dinas (Transportasi, Hotel, Uang Makan)' => 'biaya_perjalanan_dinas',
                                        'Biaya Service Kendaraan' => 'biaya_service_kendaraan',
                                        'Biaya Stationery' => 'biaya_stationery',
                                        'Biaya Lain-lain' => 'biaya_lain_lain',
                                    ];

                                    foreach ($list_kategori as $label => $key):
                                        $jumlah = $pengeluaran[$key];
                                        $ket    = $pengeluaran['ket_'.$key];
                        ...
                        <!-- TABEL PENGELUARAN -->
                        <div class="table-responsive tabel-pengeluaran-container mt-3">
                            <table class="table table-sm table-bordered">
                                <thead class="table-light">
                                    <tr>
                                        <th style="width: 50%">Uraian</th>
                                        <!-- <th style="width: 50%">Keterangan</th> -->
                                        <th style="width: 50%" class="text-end">Jumlah (Rp)</th>
                                    </tr>
                                </thead>
                                ...
                                <tbody>
                                    ...
                                    <tr class="table-active fw-bold">
                                        <td>Total</td>
                                        <!-- <td></td> -->
                                        <td class="text-end"><?= number_format($total_pengeluaran) ?></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
...
function exportBiopToExcel(fileName) {
    ...
    const headerData = [
        ...
        [],
        ['Nama:', <?= json_encode($get_biop->pic_name); ?>],
        ['Jabatan:', <?= json_encode($get_biop->jabatan) ?>],
    ...
    // Header tabel
    excelData.push(['Tanggal', 'Keterangan', 'Tol', 'Parkir', 'BBM KM', 'BBM Liter', 'BBM Rp', 'Makan', 'Jamuan', 'Meeting', 'Perjalanan Dinas', 'Service', 'Stationery', 'Lain', 'Total', 'Tempat']);
    ...
    <?php foreach ($get_data_biop_grouped as $row): ?>
        <?php
            ...
            $keterangan_escaped = json_encode(implode(", ", $row->keterangan_biaya));
            $tempat_escaped = json_encode(ucfirst($row->keterangan_tempat));
        ?>
        excelData.push([
            <?= json_encode(date('Y-m-d', strtotime($row->tanggal))) ?>, // Format tanggal untuk Excel (YYYY-MM-DD)
            <?= $keterangan_escaped ?>,
            <?= isset($row->tol)?$row->tol:0 ?>,
            <?= isset($row->parkir)?$row->parkir:0 ?>,
            <?= isset($row->bbm_km)?$row->bbm_km:0 ?>,
            <?= json_encode($row->bbm_liter ?: 0) ?>,
            <?= isset($row->bbm_rp)?$row->bbm_rp:0 ?>,
            <?= isset($row->makan)?$row->makan:0 ?>,
            <?= isset($row->jamuan)?$row->jamuan:0 ?>,
            <?= isset($row->meeting)?$row->meeting:0 ?>,
            <?= isset($row->perjalanan_dinas)?$row->perjalanan_dinas:0 ?>,
            <?= isset($row->service_kendaraan)?$row->service_kendaraan:0 ?>,
            <?= isset($row->stationery)?$row->stationery:0 ?>,
            <?= isset($row->lain)?$row->lain:0 ?>,
    ...
    excelData.push([
        ...
        <?= $total_service_kendaraan ?>, <?= $total_stationery ?>, <?= $total_lain ?>, 
        <?= $grand_total ?>, ''
    ]);
...
function exportJamuanToExcel(fileName) {
    ...
    foreach ($get_data_biop as $biop) :
        ...
        perjamuanData.push([
            <?= $no++ ?>,
            <?= json_encode(date('Y-m-d', strtotime($biop->tanggal))) ?>, // Gunakan json_encode
            <?= json_encode($biop->jamuan_tempat) ?>,                         // Gunakan json_encode
            <?= json_encode($biop->jamuan_alamat) ?>,                         // Gunakan json_encode
            <?= json_encode($biop->jamuan_jenis) ?>,
    ...
    foreach ($get_data_biop as $biop) :
        ...
        relasiData.push([
            <?= $no++ ?>,
            <?= json_encode($biop->jamuan_nama_perusahaan) ?>,             // Gunakan json_encode
            <?= json_encode($biop->jamuan_pic) ?>,                         // Gunakan json_encode
            <?= json_encode($biop->jamuan_pic_jabatan) ?>,                 // Gunakan json_encode
            <?= json_encode($biop->jamuan_jenis_perusahaan) ?>             // Gunakan json_encode
        ]);
    <?php }
    endforeach; ?>
...
document.getElementById('exportPdf').addEventListener('click', function() {
    ...
    // FIX: agar tabel tidak terpotong (hilangkan overflow)
    ...
    html2canvas(content, {
        ...
        // Kembalikan overflow table
        document.querySelectorAll(".table-responsive").forEach(el => {
            el.style.overflow = el.dataset.oldOverflow || "auto";
        });
