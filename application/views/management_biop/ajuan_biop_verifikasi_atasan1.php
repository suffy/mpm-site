    ...
    <div class="col-md-12">
        <div class="row mt-2">
            <div class="col-md">
                ...
                <form action="<?= base_url($url_atasan1_proses) ?>" method="post">
                    <div class="biop-form">
                        <div class="row mt-4">
                            <h5>Nomor Ajuan: <?= $get_biop->no_ajuan; ?></h5>
                            <div class="table-responsive">
<table id="tabel-data-biop" class="display">
        <thead>
            <tr>
        ...
        <tbody>
            <?php foreach ($get_data_biop as $biop) { ?>
                <tr>
                    ...
                    <td class="col-biaya-adjustment">
                        <input type="hidden" name="id_detail[]" id="id_detail" value="<?= $biop->id; ?>">
                        <input type="number" class="form-control" name="biaya_atasan1[]" max="<?= $biop->biaya_admin_biop;?>" id="biaya_atasan1" value="<?= $biop->biaya_atasan1 != null ? $biop->biaya_atasan1 : $biop->biaya_admin_biop ; ?>" >
                    </td>
                    <td class="col-keterangan">
                        <textarea class="form-control" name="keterangan_atasan1[]" id="keterangan_atasan1" placeholder=""><?= $biop->keterangan_atasan1 ?></textarea>
                    </td>
                    <td class="col-status">
                        <Select class="form-control" name="status[]" id="status" required>
                            <option value=""> -- Pilih -- </option>
                            <option value="0" <?= ($biop->flag_tolak_atasan1 ? $biop->flag_tolak_atasan1 : $biop->flag_tolak_admin_biop) == "0" ? "selected" : "" ?>>ok</option>
                            <option value="1" <?= ($biop->flag_tolak_atasan1 ? $biop->flag_tolak_atasan1 : $biop->flag_tolak_admin_biop) == "1" ? "selected" : "" ?>>no</option>
                        </Select>
                    </td>
                </tr>
            <?php } ?>
...
<script>
    function toggleKonten() {
        ...
        if (form.classList.contains('show')) {
            tombol_form.textContent = 'Close Detail BIOP';
        } else {
            tombol_form.textContent = 'Lihat Detail BIOP';
        }
    }
</script>