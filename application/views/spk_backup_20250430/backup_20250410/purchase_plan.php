                        ...
                        <div class="form-inline row">
                            <div class="col-sm-12">
                                ...
                                <form action="<?= base_url($url_import) ?>" method="post" enctype="multipart/form-data">
                                    ...
                                    <p>Silakan klik link untuk <a href="<?= base_url('spk/template_purchase_plan'); ?>">download</a> template</p>
                                </form>
                            </div>
                        </div>

                        <div class="row mt-5">
                            <h5>Tabel Purchase Plan</h5>

                            <form action="<?= base_url($url); ?>" method="post">
                                <table id="tabel-data">
                                    <thead>
                                        <tr>
                                            <th class="text-center" style="text-align: center" width="5%">
                                                <input type="checkbox" value="" id="select-all"><label for="select-all">Click All</label>
                                            </th>
                                            <th width="10%">Bulan</th>
                                            <th width="10%">Site_code</th>
                                            <th width="10%">KodeProduk</th>
                                            <th width="20%">NamaProduk</th>
                                            <th width="10%">PP_unit</th>
                                    </thead>
                                    <tbody>
                                        <?php foreach ($data_purchaseplan->result() as $key): ?>
                                            <tr>
                                                <td>
                                                    <center>
                                                        <input type="checkbox" id="<?= $key->id; ?>" class="select-row" name="options[]" value="<?= $key->id; ?>">
                                                    </center>
                                                </td>
                                                <td><?= $key->bulan; ?></td>
                                                <td><?= $key->nama_comp . " ($key->site_code)"; ?></td>
                                                <td><?= $key->kodeprod; ?></td>
                                                <td><?= $key->namaprod; ?></td>
                                                <td>
                                                    <input type="number" class="form-control" name="<?= $key->id; ?>" value="<?= $key->pp_unit; ?>" min='0'>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                ...
                                <div style="text-align: center;">
                                    <button type="submit" class="btn btn-submit-orange" id="update">Simpan Perubahan</button>
                                </div>
                            </form>
                        </div>
                    </div>
