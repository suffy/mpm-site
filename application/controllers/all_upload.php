...
class All_upload extends MY_Controller
{
    ...
    function all_upload()
    {
        ...
        $this->load->model(array('model_upload','model_proses_data'));
        $this->load->database();
    ...
    public function view_upload(){
      ...
      $data['query'] = $this->model_upload->cek_upload_terakhir($data);                          
    ...
    public function file_upload(){
      ...
      //konfigurasi upload zip
        ...
        // $config['upload_path'] = 'assets/uploads/zip/202010';
    ...
    //    echo "aaa";
       $data['id'] = $this->session->userdata('id');       
       $data['query'] = $this->model_upload->cek_upload_terakhir($data);
       $data['cek'] = $this->model_upload->cek_kesesuaian_upload($data);
       $data['page_content'] = 'upload/upload_form_view';
       $data['menu']=$this->db->query($this->querymenu);
       $this->template($data['page_content'],$data); 
    ...
    {   
        ...
        $status_closing = $this->uri->segment(8);        
        ...
        $this->load->library('upload', $config);
        $zip = new ZipArchive;
        //$file = "C:/xampp/htdocs/cisk/assets/uploads/zip/201705/".$filename;
        ...
        $data['lastupload']=date('Y-m-d H:i:s');
        ...
        $upl['tahun']=$year;
        $upl['status']='0';
        $upl['status_closing']=$status_closing;
        $upl['tanggal']=$tanggal;
        ...
        if($proses){
            ...
            if($nocab == '95'){
                ...
                $query= $this->model_proses_data->proses_data($data);
                $data['omzet'] = $query['omzet'];
                $data['id'] = $query['id'];
            //}
    ...
    private function console($filename,$year='')
    {
        ...
        public function prosesOmzet(){
            ...
            $data['page_content'] = 'upload/submitOmzet';
            $data['query'] = $this->model_proses_data->submitOmzet($data);
            $data['url'] = 'all_sales/sales_product_per_class_hasil/';
            $data['page_title'] = 'Submit Omzet';
            $data['menu']=$this->db->query($this->querymenu);
            $this->template($data['page_content'],$data); 
