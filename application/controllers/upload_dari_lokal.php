...
class Upload_dari_lokal extends MY_Controller
{
    ...
    function do_upload()
    {
        ...
        else
        {
            $upload_data = $this->upload->data();
                        ...
                        if ($zip->open($file) === TRUE) {
                         $zip->extractTo('./assets/uploads/zip/'.date('Ym').'/'.substr($filename,0,-4).' -d ./assets/uploads/unzip/'.substr($filename,2,2).'/ 2>&1');
                         $zip->close();
                         echo 'berhasil di ekstrak<br />';
                        } else {
                            echo 'gagal ekstrak';
                        }                           
                        */
        ...
        private function unzip($filename)
        {
            ...
            //batas suffy nonaktifkan

            /*
            $zip = new ZipArchive;
            if ($zip->open($upload_data) === TRUE) {
             $zip->extractTo('./assets/uploads/zip/'.date('Ym').'/'.substr($filename,0,-4).' -d ./assets/uploads/unzip/'.substr($filename,2,2).'/ 2>&1');
             $zip->close();
             echo 'berhasil di ekstrak<br />';
            } else {
