...
class Master_data extends MY_Controller
{
    ...
    function master_data()
    {
        ...
        $this->load->library(array('table','template','Excel_generator', 'form_validation','email'));
        $this->load->helper(array('csv','url'));
        $this->load->model(array('model_outlet_transaksi', 'model_penta', 'M_menu', 'model_request', 'model_master_data', 'model_per_hari', 'model_sales_omzet', 'model_relokasi'));
        $this->created_at = $this->model_outlet_transaksi->timezone();
        $this->userid = 999;
    }
    function index()
    ...
    //     // $limit = 50000; // 50k per chunk
    //     $limit = 5; // 50k per chunk
    //     $offset = 0;
    //     $filename = 'raw_data_' . date('Ymd_His') . '.csv';
    ...
    //     // Write header
    //     fputcsv($output, array('site_code', 'kodeprod', 'banyak')); // Sesuaikan
    ...
    //         $query = "
    //             select concat(a.kode_comp, a.nocab) as site_code, a.kodeprod, a.banyak
    //             from data2025.fi a 
    //             where a.bulan = 8
    ...
    //     $output = fopen('php://output', 'w');
    //     fputcsv($output, array('site_code', 'kodeprod', 'banyak'));
        
    //     // Gunakan unbuffered query untuk data besar
    //     $query = $this->db->query("
    //         SELECT CONCAT(a.kode_comp, a.nocab) as site_code, a.kodeprod, a.banyak
    //         FROM data2025.fi a 
    //         WHERE a.bulan = 8
    ...
    public function export_raw_data() 
    {
        ...
        // Headers harus sebelum output apapun
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="' . $filename . '"');
        ...
        // Write header
        fputcsv($output, array('kode_comp', 'kodeprod'));
        
        $total_processed = 0;
        
        do {
            // PERBAIKAN: Hanya 2 parameter untuk LIMIT dan OFFSET
            $query = "
                SELECT a.kode_comp, a.kodeprod
                FROM data2025.fi a
                LIMIT ? OFFSET ?
            ";
            
            $data = $this->db->query($query, array($limit, $offset))->result_array();
            ...
            // Bersihkan output buffer secara berkala
            if (ob_get_level() > 0) {
                ob_flush();
            }
            flush();
        ...
        // Log untuk debugging
        error_log("Export completed. Total rows: " . $total_processed);
        exit;
    }
