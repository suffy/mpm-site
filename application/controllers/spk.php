...
class Spk extends MY_Controller
{
  public function __construct()
  {
    ...
    $this->load->model(array('model_outlet_transaksi', 'model_spk', 'model_master_data', 'model_inventory', 'model_management_sales','model_management_office'));
    $this->email_tim ='anitria889@gmail.com, suffy.yanuar@gmail.com, millarosianad2@gmail.com';
    $this->created_at = $this->model_outlet_transaksi->timezone();
    $this->userid = $this->session->userdata('id');
    $this->level = $this->session->userdata('level');
    $this->username = $this->session->userdata('username');
    $this->admin = ['suffy', 'milla', 'fakhrul', 'tria', 'melinda'];
    ...
    public function dashboard()
    {
        ...
        $this->render('spk/dashboard', $data);
    }
    ...
    public function form_pesanan()
    {
        ...
        $this->render('spk/form_pesanan', $data);
    ...
    public function keranjang_belanja($signature = "")
    {
        ...
        $this->render('spk/keranjang_belanja', $data);
    }
    ...
    public function pengiriman()
    {
        ...
        }else
        {
            ...
            // echo "jml_karton : ".json_encode($jml_karton);
            // echo "id_detail : ".json_encode($id_detail);
            // echo "id_header : ".json_encode($id_header);
        ...
        $this->render('spk/pengiriman', $data);
    }
  ...
  public function preview_spk()
  {
    ...
    $bulan = date('Y-m');
    // die;
    $get_data = $this->model_spk->get_temp_spk_join_temp_spk_detail_by_userid_with_jumlah_karton($this->session->userdata('id'), $kode_alamat, $bulan);
    // die;

    // cek jika ditemukan $get_data->row()->selisih < 0, maka $status_textarea menjadi true
    $status_textarea = 0;
    ...
    $this->render('spk/preview_spk_new', $data);
  }
    ...
    public function submit_spk()
    {
        ...
        $email      = $this->input->post('email');
        $id_header  = $this->input->post('id_header');
        $alasan_order  = $this->input->post('alasan_order');

        // echo "alasan_order : ".$alasan_order;
        // die;

        $get_temp_spk_join_temp_spk_detail_by_userid_with_jumlah_karton = $this->model_spk->get_temp_spk_join_temp_spk_detail_by_userid_with_jumlah_karton($this->session->userdata('id'),$kode_alamat, date('Y-m'));
    ...
    public function keranjang_alokasi($signature = "")
    {
        ...
        // $this->load->view('kalimantan/footer');
        $this->render('spk/keranjang_alokasi', $data);
    }
    ...
    public function preview_alokasi()
    {
        ...
        // $this->load->view('kalimantan/footer');
        $this->render('spk/preview_alokasi', $data);
    }
    ...
    public function form_generate_average_sales()
    {
        ...
        $this->render('spk/form_generate_average_sales', $data);
    }
  ...
  public function list_order()
  {
      ...
      $data = [
          'title'     => 'List Order',
          'url'       => 'spk/list_order',
          'get_data'  => $this->model_spk->get_po_new($advanced),
          'from'      => ($this->input->post('from')) ? $this->input->post('from') : '',
          'to'        => ($this->input->post('to')) ? $this->input->post('to') : '',
          'limit'     => ($this->input->post('limit')) ? $this->input->post('limit') : '',
          'flag_delete' => $flag_delete
      ...
      $this->render('spk/list_order', $data);
  }

  public function list_order_detail($signature)
  {
    if (!in_array($this->session->userdata('username'), $this->admin))
    {
    ...
    }else
    {
      ...
      $alasan_order = $get_po->row()->alasan_order;
      $alasan_rilis = $get_po->row()->alasan_rilis;
      
      $tglpesan = date("Y-m", strtotime($get_po->row()->tglpesan));
      // echo "tglpesan : ".$tglpesan;
    }

    $get_data_detail = $this->model_spk->get_po_detail_by_id_po_include_delete_join_pp($id_po, $kode_alamat, $tglpesan);

    // jika di temukan selisih < 0, maka $flag_selisih = 1
    $flag_selisih = 0;
    ...
    $spk_pp_karton = $get_data_detail->row()->spk_pp_karton;
    ...
    $data = [
        ...
        'alasan_order'  => $alasan_order,
        'alasan_rilis'  => $alasan_rilis,
        'flag_selisih'  => $flag_selisih,
        'spk_pp_karton' => $spk_pp_karton
    ];    
    $this->render('spk/list_order_detail', $data);
  }
    ...
    public function rilis_po()
    {
        ...
        $alasan_rilis =  $this->input->post("alasan_rilis");

        if (!in_array($this->session->userdata('username'), $this->admin))
        {
          $this->session->set_flashdata("pesan", "Rilis Gagal. User anda tidak diizinkan me-rilis po");
          redirect('spk/list_order_detail/'.$signature);
        }
    ...
    public function update_po()
    {
        ...
        if (!in_array($this->session->userdata('username'), $this->admin))
        {
          $this->session->set_flashdata("pesan", "Update PO Gagal. User anda tidak diizinkan me-update po");
          redirect('spk/list_order_detail/'.$signature);
        }
    ...
    public function form_generate_po_outstanding()
    {
        ...
        $this->render('spk/form_generate_po_outstanding', $data);
    ...
    public function master_principal()
    {
        ...
        $this->render('spk/master_principal', $data);
    }
    ...
    public function master_produk()
    {
        ...
        $this->render('spk/master_produk', $data);
    }
    ...
    public function email_po($signature)
    {
        ...
        $send = $this->model_relokasi->send_email($email_data, 'spk');
        if ($send)
        {
            $this->session->set_flashdata("pesan_success", "Send Email PO Berhasil");
            redirect('spk/list_order_detail/'.$signature);
    ...
    {
        $data = [
            'title' => 'Purchase Plan (Export & Import)',
            'url_import'   => 'spk/import_purchase_plan',
            'url_export'   => 'spk/export_purchase_plan',
        ...
        $this->render('spk/purchase_plan', $data);
    }

    public function export_purchase_plan()
    {
        $month = $this->input->post('month');
        $query = $this->model_spk->get_purchase_plan_by_month($month);

        $this->excel_generator->set_query($query);
        $this->excel_generator->set_header(array
        (
            'site_code', 'branch_name', 'nama_comp', 'kodeprod', 'namaprod', 'pp_karton'
        ));
        $this->excel_generator->set_column(array
        (
            'site_code', 'branch_name', 'nama_comp', 'kodeprod', 'namaprod', 'pp_karton'
        ...
        $this->excel_generator->exportTo2007('Purchase Plan');
    ...
    {
        $query = "
            select '' as bulan, '' as site_code, '' as kodeprod, '' as pp_karton
        ";
        $hasil = $this->db->query($query);
        ...
        $this->excel_generator->set_header(array
        (
            'bulan (yyyy-mm)', 'site_code', 'kodeprod', 'pp_karton'
        ));
        $this->excel_generator->set_column(array
        (
            'bulan', 'site_code', 'kodeprod', 'pp_karton'
        ));
        $this->excel_generator->set_width(array(15,15,15,15));
        ...
        if ($this->upload->do_upload('file'))
        {
            ...
            foreach ($object->getWorksheetIterator() as $worksheet)
            {
                ...
                for ($row = 2; $row <= $highestRow; $row++)
                {
                    ...
                    $kodeprod   = trim($worksheet->getCellByColumnAndRow(2, $row)->getValue());
                    $pp_karton    = trim($worksheet->getCellByColumnAndRow(3, $row)->getValue());
                    ...
                    if (!$get_data->num_rows() > 0)
                    {
                        ...
                        $this->model_spk->insert_purchase_plan($data);
                    }else{
                        $data = [
                            "pp_karton"     => $pp_karton,
                            'updated_at'    => $this->created_at,
                        ...
                        $this->model_spk->update_spk_purchase_plan($data, $get_data->row()->id);
                    }
                }
  ...
  public function update_pp_po($signature)
  {
    ...
    {        
        ...
        if ($from && $to && $principal) {
            ...
            {
                $get_site_code_by_userid = $this->model_spk->get_site_code_by_userid($this->session->userdata('id'));
                $get_status_ho = $get_site_code_by_userid->row('status_ho');
                // echo $get_status_ho;
                // die;
                $flag_penta = "";
                if ($get_status_ho == 1) {
                    $sub = $get_site_code_by_userid->row('sub');

                    $site_list = $this->model_spk->get_site_by_sub($sub)->result();
                    $site_codes = [];
                    foreach ($site_list as $row) {
                        $site_codes[] = $row->site_code;
                    }

                    // $site_list = $this->model_spk->get_site_by_sub($sub)->result_array();
                    // $site_codes = array_column($site_list, 'site_code');

                    // Ubah jadi array of string kutip, Gabung jadi satu string untuk IN (...)
                    $site_code = "'" . implode("','", $site_codes) . "'";
                    // echo $site_code;die;
                }else{
            ...
            if ($principal == '001') {
                return $this->export_deltomed('po_outstanding_deltomed '.$from.'-'.$to, $site_code, $from, $to, $flag_penta);
        ...
        $this->render('spk/po_outstanding', $data);
    }

    public function export_deltomed($filename, $site_code, $from, $to)
    {
        // jika level 4 atau dp, maka tidak bisa membuka data penta

        if($this->level == 4 || $this->level == 5)
        {
            $params = "and a.kode_alamat in ($site_code)";
        }else{
            $params = "and (a.kode_alamat in ($site_code) or a.kode_alamat like 'PENTA%')";
        }
        ...
        $query = "
            ...
            where date(a.tglpo) between '$from' and '$to' $params";
        // echo "<pre>";
        // print_r($query);
        // echo "</pre>";
        // die;
    ...
    //         'fulfilment', 'leadtime_proses_do',
    //         'po_ref', 'note', 'tanggal_terima', 'leadtime_proses_kirim',
    //         'outstanding_po', 'kode_alamat'
    //     );

    //     // First pass: find maximum number of surat_jalan items
    //     foreach ($hasil->result_array() as $row) {
    //         $data_surat_jalan = json_decode($row['data_surat_jalan'], true);
    //         if (is_array($data_surat_jalan)) {
    //             $max_surat_jalan = max($max_surat_jalan, count($data_surat_jalan));
    //         }
    //     }

    //     // Add dynamic headers for surat_jalan
    //     for ($i = 1; $i <= $max_surat_jalan; $i++) {
    //         $headers[] = 'nodo_' . $i;
    //         $headers[] = 'tgl_do_' . $i;
    //         $headers[] = 'kodeprod_' . $i;
    //         $headers[] = 'qty_' . $i;
    //         $headers[] = 'tanggal_terima_' . $i;
    //     }
    ...
    //                 in_array($header, array('branch_name', 'nama_comp', 'company', 'tglpo', 'nopo', 'tipe',
    //                 'kodeprod', 'namaprod', 'qty_po', 'qty_pemenuhan', 'harga',
    //                 'value_po', 'value_pemenuhan', 'berat', 'volume',
    //                 'fulfilment', 'leadtime_proses_do',
    //                 'po_ref', 'note', 'tanggal_terima', 'leadtime_proses_kirim',
    ...
    //         // Process data_surat_jalan JSON
    //         $data_surat_jalan = json_decode($row['data_surat_jalan'], true);
    //         if (is_array($data_surat_jalan)) {
    //             $counter = 1;
    //             foreach ($data_surat_jalan as $surat) {
    //                 $new_row['nodo_' . $counter] = isset($surat['nodo']) ? $surat['nodo'] : '';
    //                 $new_row['tgl_do_' . $counter] = isset($surat['tgldo']) ? $surat['tgldo'] : '';
    //                 $new_row['kodeprod_' . $counter] = isset($surat['kodeprod']) ? $surat['kodeprod'] : '';
    //                 $new_row['qty_' . $counter] = isset($surat['qty']) ? $surat['qty'] : '';
    //                 $new_row['tanggal_terima_' . $counter] = isset($surat['tanggal_terima']) ? $surat['tanggal_terima'] : '';
    //                 $counter++;
    //             }
    //         }

    //         // Fill empty columns for rows with fewer surat_jalan items
    //         for ($i = count($data_surat_jalan) + 1; $i <= $max_surat_jalan; $i++) {
    //             $new_row['nodo_' . $i] = '';
    //             $new_row['tgl_do_' . $i] = '';
    //             $new_row['kodeprod_' . $i] = '';
    //             $new_row['qty_' . $i] = '';
    //             $new_row['tanggal_terima_' . $i] = '';
    //         }

    //         $processed_data[] = $new_row;
    //     }

    //     // Convert processed data to format suitable for excel_generator
    //     $processed_result = array();
    //     foreach ($processed_data as $row) {
    //         $processed_result[] = (object) $row;
    //     }

    //     // Create custom result object
    //     $custom_result = new stdClass();
    //     $custom_result->result_array = function() use ($processed_data) {
    //         return $processed_data;
    //     };
    //     $custom_result->result = function() use ($processed_result) {
    //         return $processed_result;
    //     };
    //     $custom_result->num_rows = count($processed_data);
    ...
    public function export_us($filename, $site_code, $from, $to)
    {
        ...
        $base_headers = array(
            ...
            'fulfilment', 'leadtime_proses_do',
            'po_ref', 'note', 'tanggal_terima', 'leadtime_proses_kirim',
            'outstanding_po', 'kode_alamat', 'data_surat_jalan', 'max_surat_jalan'
        );
        ...
        foreach ($hasil->result_array() as $row) {
            ...
            if (is_array($data_surat_jalan)) {
                // $max_surat_jalan = max($max_surat_jalan, count($data_surat_jalan));
                $global_max_surat_jalan = max($global_max_surat_jalan, count($data_surat_jalan));
            }
        }

        // echo "global_max_surat_jalan : " . $global_max_surat_jalan;
        // die;

        // Add dynamic headers for surat_jalan
        $dynamic_headers = array();
        for ($i = 1; $i <= $global_max_surat_jalan; $i++) {
            $dynamic_headers[] = 'nodo_' . $i;
            $dynamic_headers[] = 'tgl_do_' . $i;
            $dynamic_headers[] = 'batch_number_' . $i;
            $dynamic_headers[] = 'kodeprod_' . $i;
            $dynamic_headers[] = 'qty_' . $i;
            $dynamic_headers[] = 'tanggal_terima_' . $i;
        }
        ...
        foreach ($hasil->result_array() as $row) {
            ...
            foreach ($base_headers as $header) {
                $new_row[$header] = isset($row[$header]) ? $row[$header] : '';
                if ($header == 'max_surat_jalan') {
                    // Hitung max_surat_jalan untuk row ini
                    $data_surat_jalan = json_decode($row['data_surat_jalan'], true);
                    $new_row[$header] = is_array($data_surat_jalan) ? count($data_surat_jalan) : 0;
                } else {
                    $new_row[$header] = isset($row[$header]) ? $row[$header] : '';
                }
            }
            ...
            if (is_array($data_surat_jalan)) {
                ...
                foreach ($data_surat_jalan as $surat) {
                    $new_row['nodo_' . $counter] = isset($surat['nodo']) ? $surat['nodo'] : '';
                    $new_row['tgl_do_' . $counter] = isset($surat['tgldo']) ? $surat['tgldo'] : '';
                    $new_row['batch_number_' . $counter] = isset($surat['batch_number']) ? $surat['batch_number'] : '';
                    $new_row['kodeprod_' . $counter] = isset($surat['kodeprod']) ? $surat['kodeprod'] : '';
                    $new_row['qty_' . $counter] = isset($surat['qty']) ? $surat['qty'] : '';
                    $new_row['tanggal_terima_' . $counter] = isset($surat['tanggal_terima']) ? $surat['tanggal_terima'] : '';
                    $counter++;
            ...
            // Fill empty columns for rows with fewer surat_jalan items
            for ($i = (is_array($data_surat_jalan) ? count($data_surat_jalan) : 0) + 1; $i <= $global_max_surat_jalan; $i++) {
                $new_row['nodo_' . $i] = '';
                $new_row['tgl_do_' . $i] = '';
                $new_row['batch_number_' . $i] = '';
                $new_row['kodeprod_' . $i] = '';
                $new_row['qty_' . $i] = '';
                $new_row['tanggal_terima_' . $i] = '';
            }
    ...
    public function import_do_deltomed()
    {
        ...
        $this->render('spk/import_do', $data);
    }
    ...
    public function insert_do_proses()
    {
        ...
        if ($this->upload->do_upload('file'))
        {
            ...
            if ($jumlahSheet > 1) {
                ...
                redirect('spk/import_do_deltomed', 'refresh');
            }
    ...
    public function konfirmasi_po()
    {
        ...
        $this->render('spk/konfirmasi_po', $data);
    }
    ...
    {    
        ...
        $this->render('spk/konfirmasi_po_detail', $data);
    }

    public function surat_jalan_mpm_deltomed($kode_surat_jalan_formatted, $signature)
    {            
        $kode_surat_jalan = str_replace("_", "/", $kode_surat_jalan_formatted);
        $get_surat_jalan_mpm_deltomed_by_kode = $this->model_spk->get_surat_jalan_mpm_deltomed_by_kode($kode_surat_jalan);
        if (!$get_surat_jalan_mpm_deltomed_by_kode->num_rows() > 0) {
            $this->session->set_flashdata("pesan", "Belum ada Surat Jalan MPM. Data not found !");
            redirect('spk/konfirmasi_po_detail/'.$signature);
        }

        $image = $get_surat_jalan_mpm_deltomed_by_kode->row()->image;
        $terima_at = $get_surat_jalan_mpm_deltomed_by_kode->row()->terima_at;
        $terima_by = $get_surat_jalan_mpm_deltomed_by_kode->row()->terima_by;
        $kode_surat_jalan = $get_surat_jalan_mpm_deltomed_by_kode->row()->kode_surat_jalan;
        $nodo = $get_surat_jalan_mpm_deltomed_by_kode->row()->nodo;
        $tgldo = $get_surat_jalan_mpm_deltomed_by_kode->row()->tgldo;
        $nopo = $get_surat_jalan_mpm_deltomed_by_kode->row()->nopo;
        $tglpo = $get_surat_jalan_mpm_deltomed_by_kode->row()->tglpo;

        $data = [
            'title'         => 'Surat Jalan MPM : '.$kode_surat_jalan,
            'get_data'      => $get_surat_jalan_mpm_deltomed_by_kode,
            'image'         => $image,
            'terima_at'     => $terima_at,
            'terima_by'     => $terima_by,
        ...
        // $this->load->view('kalimantan/header_full_width', $data);
        // $this->load->view('management_claim/css');
        // $this->load->view('spk/surat_jalan_mpm_deltomed', $data);
        // $this->load->view('kalimantan/footer');

        $this->render('spk/surat_jalan_mpm_deltomed', $data);
    }    
    ...
    public function export_konfirmasi_po_detail($signature)
    {
        ...
        if ($supp == 005) {
            ...
            $this->excel_generator->set_header(array
            (
                'tgldo', 'nodo', 'batch_number', 'ed', 'kodeprod', 'namaprod', 'qty', 'tanggal_terima (isi disini)'
            ));
            $this->excel_generator->set_column(array
            (
                'tgldo', 'nodo', 'batch_number', 'ed', 'kodeprod', 'namaprod', 'qty', 'tanggal_terima'
            ));

            $this->excel_generator->set_width(array(15,15,15,15,15,15,15,15));
            $this->excel_generator->exportTo2007('Export Konfirmasi DO');
        }
    }
    ...
    public function lead_time()
    {
        ...
        $data = [
            'title'     => 'Lead Time (Timetable Pengiriman)',
            'url'       => 'spk/lead_time',
            'get_data' => '',
            'get_principal' => $this->model_spk->get_principal($this->session->userdata('supp')),
        ];
    ...
    public function export_lead_time_deltomed($filename, $site_code, $from, $to)
    {
        ...
        {
            $params_penta = "or a.kode_alamat like 'PENTA%'";   
        }else{
            $params_penta = "";
        }
        ...
        $query = "
            select 	a.kode_alamat, c.branch_name, c.nama_comp, date(a.tglpo) as tglpo, a.nopo,
                    ...
                    date(a.tglpesan) as tglpesan,
                    datediff(a.tglpo, a.tglpesan) as lt_create_po, a.tipe, a.total_value
            ...
            and (a.kode_alamat in ($site_code) $params_penta )
        ";
        // echo "<pre>";
        // print_r($query);
        // echo "</pre>";
        ...
        $this->excel_generator->set_header(array
        (
            'kode_alamat', 'branch_name', 'nama_comp','tglpesan', 'tglpo','nopo', 'lt_create_po', 'nodo', 'tgldo','lt_podo','tanggal_terima','lt_kirim_barang','lt_total', 'tipe', 'total_value'
        ));
        $this->excel_generator->set_column(array
        ( 
            'kode_alamat', 'branch_name', 'nama_comp', 'tglpesan', 'tglpo','nopo', 'lt_create_po', 'nodo', 'tgldo','lt_podo','tanggal_terima','lt_kirim_barang','lt_total', 'tipe', 'total_value'
        ));
        $this->excel_generator->set_width(array(15,15,15,15,15,15,15,15,15,15,15,15,15,15,15)); 
        $this->excel_generator->exportTo2007($filename); 
    }
    ...
    public function import_do_us()
    {
        ...
        $this->render('spk/import_do_us', $data);
    }
    ...
    public function insert_do_us_proses()
    {
        ...
        // cek directory
        if(!file_exists('./assets/uploads/do/do_us/'.date('Ym'))){
            mkdir('./assets/uploads/do/do_us/'.date('Ym'), 0777, true);
        }
                    ...
                    if ($detail) 
                    {
                        ...
                        while (($line = fgets($detail)) !== false) 
                        {
                            ...
                            $data = [
                                ...
                                'batch_number'    => str_replace('"','',$lineArr[14]),
                                'ed'    => str_replace('"','',$lineArr[15]),
                                'userid'    => $id,
                                'lastupdate' => $created
                            ];
    ...
    public function list_surat_jalan_us($tgldo)
    {
        ...
        // $this->load->view('kalimantan/footer');
        $this->render('spk/list_surat_jalan_us', $data);
    }
    ...
    public function edit_surat_jalan_us($kode_surat_jalan_formatted)
    {
        ...
        // $this->load->view('kalimantan/footer');
        $this->render('spk/edit_surat_jalan_us', $data);
    ...
    public function helpdesk()
    {
        ...
        if ($username == 'suffy' || $username == 'tria' || $username == 'melinda' || $username == 'yuniah' || $username == 'april_deltomed') 
        {
            if ($this->input->post('type') == 2) {
                // Export data
                $this->export_history_helpdesk($this->input->post('from'), $this->input->post('to'), $this->input->post('status'), $this->supp);
                // $this->model_spk->export_history_helpdesk($this->input->post('from'), $this->input->post('to'), $this->input->post('status'), $this->supp);
                // exit();
            } else {
                $data_helpdesk = $this->model_spk->history_helpdesk($form_search, $this->supp);
            }
        } else {
            if ($this->input->post('type') == 2) {
                // Export data
                $this->export_history_helpdesk_by_userid($this->input->post('from'), $this->input->post('to'), $this->input->post('status'), $this->supp, $userid);
                // $this->model_spk->export_history_helpdesk_by_userid($this->input->post('from'), $this->input->post('to'), $this->input->post('status'), $this->supp, $userid);
                // exit();
            } else {
                $data_helpdesk = $this->model_spk->history_helpdesk_by_userid($userid, $form_search, $this->supp);
            }
        }
    ...
    public function helpdesk_insert()
    {
        ...
        $post_header = [
            ...
            'signature' => 'HLP-' . rand() . md5($this->model_outlet_transaksi->timezone()) . date('Ymd'),
            'created_by' => $this->session->userdata('id'),
            'created_at' => $this->model_outlet_transaksi->timezone()
        ];
        ...
        $post_detail = [
            ...
            'signature' => 'HLP-msg' . rand() . md5($this->model_outlet_transaksi->timezone()) . date('Ymd'),
            'created_by' => $this->session->userdata('id'),
            'created_at' => $this->model_outlet_transaksi->timezone()
        ];
    ...
    public function helpdesk_email($signature)
    {
        ...
        $get_email = $this->model_spk->get_email_by_userid($this->userid);
        if ($get_email->num_rows() < 0) {
            $this->session->set_flashdata("pesan", "Data email tidak ditemukan !");
            redirect('spk/helpdesk');
        }
        $email_to = $get_email->row('email');
        // echo 'email : ' .$email_to; die;
        ...
        $from = "tria@muliaputramandiri.com";
        $to = $email_to. ',' .$get_helpdesk->row('email');
        // $cc = "milla@muliaputramandiri.com";
        $cc = "linda@muliaputramandiri.com,tria@muliaputramandiri.com,suffy.yanuar@gmail.com,ilham@muliaputramandiri.com,fakhrul@muliaputramandiri.com, millarosianad2@gmail.com";
        $subject = "MPM Site|Helpdesk - Pengajuan Helpdesk : " . $get_helpdesk->row('no_tiket');
        ...
        $this->email->to($to);
        $this->email->cc($cc);
        $this->email->reply_to('tria@muliaputramandiri.com');
        $this->email->subject($subject);
        $this->email->message($message);
    ...
    public function helpdesk_detail($signature)
    {
        ...
        $this->render('spk/helpdesk_detail', $data);
    }
    ...
    public function export_history_helpdesk($from, $to, $status, $supp)
    {
        $form_search = [
            'from'   => $from,
            'to'     => $to,
        ...
        $data_helpdesk = $this->model_spk->history_helpdesk($form_search, $supp)->result_array();
        ...
        header('Content-Disposition: attachment; filename=Export_Helpdesk.csv');
    ...
    public function export_history_helpdesk_by_userid($from, $to, $status, $supp, $userid)
    {
        $form_search = [
            'from'   => $from,
            'to'     => $to,
        ...
        $data_helpdesk = $this->model_spk->history_helpdesk_by_userid($userid, $form_search, $supp)->result_array();
    ...
    public function list_surat_jalan_deltomed($tgldo)
    {
        ...
        $this->render('spk/list_surat_jalan_deltomed', $data);
    }
    ...
    //     $data = [
    //         'title'     => 'Analisa Piutang '. $tanggal,
    //         'url'       => 'spk/analisa_piutang',
    //         'url_export'    => "spk/export_analisa_piutang",
    //         'url_export_detail'    => "spk/export_analisa_piutang_detail",
    //         'get_data'  => $get_data,
    //         'tanggal'   => $tanggal
    //     ];
    //     $this->navbar($data);
    //     $this->load->view('kalimantan/header_full_width', $data);
    //     $this->load->view('management_claim/css');
    //     $this->load->view('spk/analisa_piutang', $data);
    //     $this->load->view('kalimantan/footer');
    // }
    ...
    public function analisa_piutang()
    {
        ...
        if($this->session->userdata('supp') !='000'){
            $this->session->set_flashdata('pesan_error', 'Anda Tidak Punya Akses');
            redirect('management_office/dashboard', 'refresh');
            die;
        }
        ...
        $this->render('spk/analisa_piutang', $data);
    }

    public function akses_kodelang()
    {

        // echo "aaA";
        // die;
        $get_region = $this->model_management_sales->get_region_by_map_akses_region_nasional($this->userid);
        if ($get_region->num_rows() > 0) {
            $region = 'all';
        }else{
            // echo "bbb";
            // die;
            $get_region = $this->model_management_sales->get_region_by_map_akses_region($this->userid);
            ...
            if ($get_region->num_rows()>0) {
                ...
                {
                    $get_site_code_by_userid = $this->model_spk->get_site_code_by_userid($this->session->userdata('id'));
                    $kode_lang = '1'.$get_site_code_by_userid->row()->kode_lang;
                    return $kode_lang;
                }
            }
    ...
    public function export_analisa_piutang()
    {
        $kodelang = $this->akses_kodelang();
        $data = $this->model_spk->export_analisa_piutang($kodelang);
    }

    public function export_analisa_piutang_detail()
    {
        $kodelang = $this->akses_kodelang();
        $data = $this->model_spk->export_analisa_piutang_detail($kodelang);
    }
