...
class PHPJasperXML {
    ...
    public function PHPJasperXML($lang="id",$pdflib="TCPDF") {
        $this->lang=$lang;
      error_reporting(0);
        $this->pdflib=$pdflib;
    }
...
//        $b=$data->subreportParameter;
    ...
    public function transferDBtoArray($host,$user,$password,$db_or_dsn_name,$cndriver="mysql") {
        $this->m=0;
        $link = !$this->connect($host,$user,$password,$db_or_dsn_name,$cndriver);
        if($link)	//connect database
        {
            echo "Fail to connect database";//.mysql_error($link);
            exit(0);
        }
